...
| D16 | MAJOR | chip 2 has no block kernels: block-8 record is chip-1-only; block-8 shipping gated on chip-2 conversion | L | **CONVERTED AND MEASURED 2026-09-01 (6d0d47b) — AND CHIP 2 DOES NOT FIT, AT ANY BLOCK SIZE.** Whole chip-2 graph, signal present, chip 1 running its full 32 strips, 983.04 MHz: **342,090 cycles/block at block 8 = 208.8% of budget; 646,390 at 16 = 197.3%; 1,258,101 at 32 = 192.0%** (261.0 / 246.6 / 240.0% at 786.432 MHz). **16 does not rescue it and 32 does not rescue it** — 8→32 moves it sixteen points out of two hundred, because the work is per-SAMPLE and the budget scales with the block. **GEQ is the wall**: 17 instances x 28 bands = 476 biquad stages per sample, 951.2 cycles/sample each = 776 MHz at 48 kHz, **79% of the part on its own**. That is a cost and not a conversion artefact — the fused cascade measures 37.2 cycles per band-sample against chip 1's fused EQ at 40.5, the same `_bq_fx_cascade_blk` on both chips. **With BOTH levers chip 1 has already proven** (biquad pairing, dynamics pairing, at their GRAPH-measured factors) the ten measured classes go 1,720 → 1,179 MHz, **still 119.9% of the part, 196 MHz over**. The shortfall is structural, not kernel tuning. Three further defects were found on the way and fixed: [[D75]] the TDM gather read past the end of a scalar slot, [[D76]] all 24 chip-2 faders shared chip 1's pool slots, [[D77]] DCA's ramp ran BLOCK times long. Verification: all 44 raw node-output probes pass and 6 of 13 meter peaks are bit-identical, but the dynamics path is NOT bit-exact — see [[D80]]. Full write-up `MW/D32/DSP/dsp4-chip2-blockkernels-20260901.md`; costs in `dsp4-function-costs.csv` (session 17). **AT THE PW GATE.** |
