...
| D16 | MAJOR | chip 2 has no block kernels: block-8 record is chip-1-only; block-8 shipping gated on chip-2 conversion | L | **CONVERTED AND MEASURED 2026-09-01 (6d0d47b) — AND CHIP 2 DOES NOT FIT, AT ANY BLOCK SIZE.** Whole chip-2 graph, signal present, chip 1 running its full 32 strips, 983.04 MHz: **342,090 cycles/block at block 8 = 208.8% of budget; 646,390 at 16 = 197.3%; 1,258,101 at 32 = 192.0%** (261.0 / 246.6 / 240.0% at 786.432 MHz). **16 does not rescue it and 32 does not rescue it** — 8→32 moves it sixteen points out of two hundred, because the work is per-SAMPLE and the budget scales with the block. **GEQ is the wall**: 17 instances x 28 bands = 476 biquad stages per sample, 951.2 cycles/sample each = 776 MHz at 48 kHz, **79% of the part on its own**. That is a cost and not a conversion artefact — the fused cascade measures 37.2 cycles per band-sample against chip 1's fused EQ at 40.5, the same `_bq_fx_cascade_blk` on both chips. **With BOTH levers chip 1 has already proven** (biquad pairing, dynamics pairing, at their GRAPH-measured factors) the ten measured classes go 1,720 → 1,179 MHz, **still 119.9% of the part, 196 MHz over**. **PW RULED ON THIS BEFORE THE GATE FIRED (751fdf8): GEQ stays, the biquad primitive gets fixed, feature cuts and chip splits are OFF the table** -- the 851 MHz figure prices our primitive, not the feature. The measurement supports that and gives the break-even: chip 2 carries **642 biquad stages/sample** and 646 MHz of non-cascade work, so **break-even is 11.0 cycles/band-sample, a 3.4x improvement** (4.6 at 786.432 MHz); **with the dynamics pairing chip 1 already has, 18.5 for today's graph and 15.1 for PW's full 31-band-on-all-outputs target -- a 2.5x improvement.** Caveat for whoever takes it: ADI's `iircas` at 1-2 c/biquad-sample does not carry this tree's 80-bit error-feedback contract (D2/D5), and the current algorithm's own instruction floor of ~11/stage/sample is to within this arithmetic exactly the break-even. Three further defects were found on the way and fixed: [[D75]] the TDM gather read past the end of a scalar slot, [[D76]] all 24 chip-2 faders shared chip 1's pool slots, [[D77]] DCA's ramp ran BLOCK times long. Verification: all 44 raw node-output probes pass and 6 of 13 meter peaks are bit-identical, but the dynamics path is NOT bit-exact — see [[D80]]. Full write-up `MW/D32/DSP/dsp4-chip2-blockkernels-20260901.md`; costs in `dsp4-function-costs.csv` (session 17). **AT THE PW GATE.** |
| D20 | MAJOR | GAIN=1MAC fold ruled, not yet implemented (−17 c/s/strip) | S | **STILL BLOCKED, and the metering half is now settled 2026-08-29 (session 5).** PW's wide-word ruling took the METER off the rounded post-trim store — the meter accumulates the MS word of GAIN's own product in register and nothing is stored for it. That does not unblock the fold, because the ruling's premise ("BLK_TAP_TRIM's only consumer is a meter") is not the graph: the ROUTER reads it for pickoff 0 and needs a Q4.28 sample, and GAIN still has to write BLK_CHAIN_B for FILT. The −17 is the round/saturate plus those two stores, so what remains of D20 is the GAIN→FILT COEFFICIENT fold (g into stage 1's b0/nh/n2) plus an on-demand materialisation of the post-trim tap for the sends that actually select pickoff 0 — a numeric-spec amendment, not a meter question. Said in the emitted node . **PRICED AT ZERO AND CLOSED, 2026-09-08 (session 27).** What remained of D20 after PW's tap ruling is deleting GAIN's store into the chain ping-pong. `DSP4_GAIN_NOCHAIN=1` replaces that store with a nop — the instruction count the fold would reach, without building the fold — so the whole-graph difference is the fold's CEILING, measured rather than counted. **261,718 → 261,743 c/blk: the difference is −25 cycles, 0.008% of budget, and it has the wrong sign. The ceiling is ZERO.** The store sits in the shadow of the float GAIN kernel's serial dependency chain — session 25's lesson that the instruction count is not the cycle count, running the other way. So the remaining half of D20 would buy nothing on the part while costing a numeric-spec amendment (g folded into stage 1's b0/n1/n2) plus an on-demand materialisation of the post-trim tap. **NOT WORTH DOING, and that is now a measurement rather than an opinion** |
...
| D22 | MAJOR | RTG rebuilds control state every block: 15–29× over floor, largest gap in the strip | M | **FIXED** f179002 — per-strip control-epoch counter bumped by the SPI handler, plus published ramp-busy words; 0 of 256 bus words differ against the pre-batch golden. **AND THE OTHER HALF IS NOW CLOSED TOO, 2026-09-08 (session 27): the 15–29× was measured, and it was the ACCUMULATE rather than the prep this finding blamed.** `DSP4_RTG_NOACC=1` deletes the per-strip crosspoint accumulate and nothing else, so the whole-graph difference IS that accumulate: **16,155 cycles/block, 4.93% of chip 1's budget, for 64 live crosspoints × 16 samples = 1,024 MACs — 15.78 cycles per MAC against a floor of about one.** The gap is the ACCUMULATOR'S ADDRESS, not the arithmetic: the crosspoint-major form loads a bus's whole 80-bit [lo, hi, ex] triple into MRF, MACs once and writes all three words back, per sample per crosspoint. **The fix is a BUS-MAJOR fabric (`DSP4_RTG_FABRIC`, default on, block kernels only): each ROUTING node publishes a COLUMN of `_xpc[bus*32 + strip]` at control rate and parks its post-fader block in `_rtg_src`; one shared pass (`src/rtg_fabric.asm`) loads each accumulator ONCE per sample and MACs every strip that feeds it.** **Measured: 289,676 → 278,938 c/blk, −10,738 = −3.28% of budget**, leaving 5,417 for the same MACs plus the parking copy — **5.29 c/MAC, a 2.98× improvement**, and the cost is now bounded by the number of BUSES rather than by patch density, so it improves further on a real mix. **Exact, not approximate**: the accumulate is 80-bit MRF integer with no rounding before `_acc64_rns28` (D1), so the sum is order-independent — `busgold.sh` GRAPH BIT-EXACT, 0 of 256, with the fabric on. Sends with a non-default pickoff read a tap that is not parked and keep the old sparse `_rtg_list` path, which is correct rather than merely unused. `DSP4_RTG_FABRIC=0` rebuilds the previous block-kernel image BYTE FOR BYTE on both chips. Cost: 164 code words, 1,379 DM words. Write-up `MW/D32/DSP/dsp4-chip1-frontier-20260908.md`; costs session 27. **Still ~5× over a one-cycle floor** — the residual is the inner loop's load-use interlock and the per-bus triple; a software-pipelined form is named and not taken |
...
| D25 | MINOR | batched small wastes: SEND copies, EQ tap loop, TUBE bypass copy, DLY addressing, dead scan | S ea | **PARTLY FIXED** f179002 — DLY circular DAG addressing (17 → 5 instr/sample), the 37 INTERCHIP_SEND copy bodies deleted, dead scan with D14. EQ tap fold and TUBE bypass NOT taken: both ~2 c/s against a slot-protocol change, costed in the session outcome. **THE DLY ITEM'S 'L2 LATENCY SHARE — NEEDS MEASUREMENT' IS NOW MEASURED, AND IT WAS THE BIGGEST SINGLE LEVER ON CHIP 1 (2026-09-08, session 27).** `DSP4_DLY_NOMEM=1` deletes the delay line's READ from the interleaved block loop and keeps everything else: **21,427 cycles/block, 6.54% of budget, 41.85 cycles per sample per strip** — so the record's 63 c/s minus 8.4 of address arithmetic was indeed memory. **The fix is not to delete anything but to STOP TURNING THE MEMORY AROUND PER SAMPLE** (`DSP4_DLY_SPLIT`, default on): write the whole block into the delay line, then read the whole block out. **Measured 289,676 → 272,819 c/blk, −16,857 = −5.14% of budget — 79% of the entire L2 read cost recovered without deleting a single read.** **Bit-exact, and the argument is at the loop**: sample k writes w+k and reads w−off+k, so a read that lands inside this block's writes wants j = k−off < k, which the interleaved order had already written; off = 0 reads its own write either way. Proved at the offset the stored golden CANNOT reach — `busgold.sh` gained `DLYOFF` and `dsp4_pairgraph.py` a `--dly`, and the split and interleaved arms captured at **DlyOff = 7** (reads landing partially inside the block's own writes) are **0 of 256 words apart**, both differing from the DlyOff = 0 golden in all 256 as the positive control. `DSP4_DLY_SPLIT=0` rebuilds the interleaved loop byte for byte |
...
| D50 | MODERATE | chip 2's `C2_AUX_DLY_*` nodes take the non-pool DLY template and so have no block kernel at all | S–M | **FIXED 2026-09-01 (6d0d47b)** — all 15 chip-2 DELAY nodes take the generic block wrapper and process every sample of the block. Measured 103.0 cycles/sample at block 32 (102.4 at block 16; the block-8 readings straddle it at 151.4 and 41.8 and are inside the instrument's cross-boot spread). Rode with D16 |
| D75 | MAJOR | `_gather_chip2` indexes `_tx_out_slot_<id>` by sample under block kernels, and `gen_output_tdm` declared it a SCALAR — the TDM gather read BLOCK words out of a one-word variable, i.e. seven neighbouring variables per output, onto the wire | S | **FIXED 2026-09-01 (6d0d47b)** — the slot is `[DSP4_BLOCK_SIZE]` under block kernels and OUTPUT_TDM walks the whole block into it. True of every chip-2 block-kernel build since the gather learned to index. Found by [[D16]] |
| D76 | MAJOR | `gen_fader_pan_fixed`'s block kernel pointed i0/i1 at `BLK_CHAIN_B`/`BLK_CHAIN_A` unconditionally — strip-POOL slots, and chip 2 has no strip pool, so all 24 chip-2 faders read and wrote the same two slots | S | **FIXED 2026-09-01 (6d0d47b)** — chip 1 keeps the pool (that is what makes its fused ping-pong free); chip 2 names its own `_blk_` buffers. In a block-kernel build every aux, group, FX, sub and main fader had been carrying whichever fader ran last. Found by [[D16]] |
| D77 | MODERATE | DCA decrements its ramp frame count by a literal 1; it has no audio path, so the chain reaches it once per SAMPLE in a per-sample build and once per BLOCK under kernels — the ramp ran BLOCK times long | S | **FIXED 2026-09-01 (6d0d47b)** — consumes a block's frames and applies a block's step under kernels, the same correction GAIN and FADER_PAN carry. A GainSafe DCA move would have taken 240 ms at BLOCK=8 instead of 30. This is the trap the D16 dispatch named; the audit found no other stale count on chip 2 |
| D78 | MAJOR | chip 2's node graph had never run on the bench: the main loop is gated on `_boot_config_received`, and every session configured chip 1 only — "BOOT_STAGE 5 is its pass mark" IS WAITCFG | S | **CLOSED 2026-09-01 (6d0d47b)** — `sigprofile2_run.sh` and `c2gold_run.sh` send chip 2 its own CONFIG_COMMIT and its pass mark becomes STAGE 7 (RUNNING). There was no chip-2 cost record to have had until this changed. Every bench script that wants chip 2 to process audio needs this |
| D79 | MAJOR | configuring chip 2 lands a stray word in a node's parameter state — `_fdr_level_C2_AUX_FDR_01` read `0xE0FE0000`, a diag header word, and that whole aux chain carried zero while its neighbours were perfect | S–M | open — the chip-1 phenomenon `gainfix.py` repairs, seen on chip 2 for the first time because nobody had ever configured chip 2 ([[D78]]). `c2gold.sh` witnesses each chain's fader head and excludes the unhealthy ones by name. **Chip 2 has no `gainfix.py` equivalent and wants one before any chip-2 bar is trusted at scale** |
| D80 | MAJOR | chip 2's per-sample and block-kernel builds differ by a systematic 0.44–0.70% on the chains whose GATE and COMP are ENGAGED (group, sub, main); peak low, RMS high | M | open, NOT root-caused — measured 2026-09-01. Real, not boot noise: the same block-kernel build booted twice gives 19 of 26 probes bit-identical and worst 0.198%, against a cross-build worst of 1.249% consistent in sign. Ruled out by evidence: the biquad cascades (aux chains carry three and agree EXACTLY), stale sidechain reads (`_gate_key_src_`/`_gate_det_src_`/`_comp_key_src_` all 0), and boot-to-boot variation. What is left is GATE and COMP under the generic wrapper. **No chip-1-grade bit-exactness is claimed for chip 2's dynamics path** |
| D81 | MAJOR | `_C2_PAIR_FAMILIES` states each chip-2 pair family as a FIXED chain of classes and drops any instance missing one of them, so adopting a new class into a family silently un-pairs every instance in a graph that does not have it yet. Adding `OGEQ` (the per-output graphic EQ, present only under `--geq-outputs mainout`) to the MOUT family dropped all four instances of the SHIPPING graph — four output compressors and limiters would have gone from paired to scalar, a capacity regression no bar in the tree catches, because the shipping graph still builds and is still bit-exact | S | **FIXED 2026-09-03 (session 26)** — `_c2_family_classes()`: a class absent from the graph entirely is dropped from the TEMPLATE, not from the family, and `paired`/`bqpaired` are filtered with it. Verified the only way that matters: the shipping `src/` regenerates FILE-FOR-FILE identical and all three W0 witnesses rebuild byte for byte |
...
| RTG | 232.6 | 8–15 | 15–29× — **was the largest gap in the strip; the accumulate half CLOSED 2026-09-08, see D22 (15.78 → 5.29 c/MAC)** | see D22: the whole control-rate section (18 send ramps, pickoff resolution, 25-crosspoint list rebuild, `C1_RTG_01.asm:100-571`) runs EVERY BLOCK for state that changes at control rate; plus `_acc64_mac_blk` reloads/stores the 64-bit accumulator per sample (D23) | dirty-flag rebuild + MRF-resident accumulate | M |
