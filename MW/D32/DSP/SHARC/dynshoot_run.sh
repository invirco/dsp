#!/bin/bash
# dynshoot_run.sh — bench half of bqprobe.sh. Boot the staged image and
# read the per-instruction cycle probe back.
#
...
# phase-slip retry. Byte for byte bqshoot_run.sh's pin hand-back and boot
# loop; check_bench_pins.sh enforces that across every run script.
set -u
cd /home/app/dspboot
...
for cycle in 1 2 3; do
  ...
  for t in 1 2 3 4 5 6; do
    OUT=$(python3 dsp4_dyn_shoot.py 2>&1); RC=$?
    case "$OUT" in
