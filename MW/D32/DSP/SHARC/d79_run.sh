#!/bin/bash
# d79_run.sh — bench half of d79.sh. Boots and configures both chips N times
# and reads the word at SPI dispatch index 0 on each chip after every one.
set -u
BOOTS="$1"; OUT="$2"
...
flock "$BENCH_LOCK_FD"
printf 'pid=%s script=d79_run.sh started=%s\n' "$$" "$(date -u +%FT%TZ)" > "$BENCH_LOCKFILE.info"
...
python3 dsp4_d79.py --boots "$BOOTS" --json "$OUT"
