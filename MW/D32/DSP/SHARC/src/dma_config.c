...
{
#if DSP4_BISECT == 32
    /* Rung 32 (2026-08-23) — descriptor-list arming with a descriptor
     * built RIGHT HERE, so arm_region()'s construction is out of the
     * picture. Rung 31 proved the channel runs clean (RUN = 2, no error)
     * when armed register-based, with either an L1-alias or an L2
     * buffer. The only remaining difference is the descriptor fetch, so
     * this is that one variable on its own: a single self-referencing
     * descriptor, every word written here. */
    {
        static volatile uint32_t d0[5];
        uint32_t d = DMA_MMR_BASE_LO;
        REG32(d + 0x30u) = 0x00000002u;        /* clear any latched error */
        d0[0] = l1_to_sys((uint32_t)&d0[0]);   /* ring: points at itself */
        d0[1] = l1_to_sys((uint32_t)REGION_A_PING);
        d0[2] = BITM_DMA_CFG_EN | BITM_DMA_CFG_WNR | ENUM_DMA_CFG_MSIZE04 |
                ENUM_DMA_CFG_PSIZE04 | ENUM_DMA_CFG_DSCLIST |
                ENUM_DMA_CFG_FETCH05;
        d0[3] = 256u;                          /* XCNT */
        d0[4] = 4u;                            /* XMOD */
        (void)d0[0];                           /* let the writes land */
        REG32(d + DMA_OFF_DSCPTR) = l1_to_sys((uint32_t)&d0[0]);
        REG32(d + DMA_OFF_CFG) = (uint32_t)d0[2];
        DIAG_STAGE(6);
        return;
    }
#endif
     ...
     * the address translation is what to fix. */
    {
        ...
        REG32(d + 0x04u) = l1_to_sys((uint32_t)REGION_A_PING); /* ADDRSTART */
        REG32(d + 0x0Cu) = 256u;                              /* XCNT */
        REG32(d + 0x10u) = 4u;                                /* XMOD */
        /* WNR set: SPORT0 half A is a RECEIVE lane, so this is a memory
         * WRITE. The first version of this probe omitted it and armed a
         * memory read, which was the one caveat on the earlier result. */
        REG32(d + DMA_OFF_CFG) =
            BITM_DMA_CFG_EN | BITM_DMA_CFG_WNR | ENUM_DMA_CFG_MSIZE04 |
