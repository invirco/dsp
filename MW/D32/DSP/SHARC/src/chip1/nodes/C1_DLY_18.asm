/*----------------------------------------------------------------------
 * Ch 18 Delay (DELAY)
 * Node ID:    C1_DLY_18
 * Chip:       1
 * Channels:   1
 * SPI Page:   1
 * SPI Addr:   2526
 * RampProfile: InstantCtl
 *
 * AUTO-GENERATED by tools/dsp/dsp_codegen.py — do not edit directly.
 *----------------------------------------------------------------------*/
#include "dsp_block.h"
#include "lib/dyn_lut.h"

        /* RampProfile: InstantCtl | Mode: Instant | Up: 0ms (0f) Down: 0ms (0f) | Curve: Linear | Scope: Scalar */

        /* DELAY: Audio delay line with dynamic shared long-delay slot */
        /* SPI page=1 addr=2526 */
        /* Local fallback: 20.0ms = 960 samples */
        /* Shared max:     250.0ms = 12000 samples */

        #include "blk_pool.h"

        .section/dm seg_delay;
.extern _buf_C1_TUBE_18;
        .extern _dly_pool_buf_00;
.extern _dly_pool_buf_01;
.extern _dly_pool_buf_02;
.extern _dly_pool_buf_03;
.extern _dly_pool_buf_04;
.extern _dly_pool_buf_05;
.extern _dly_pool_buf_06;
.extern _dly_pool_buf_07;
.extern _dly_pool_wptr_00;
.extern _dly_pool_wptr_01;
.extern _dly_pool_wptr_02;
.extern _dly_pool_wptr_03;
.extern _dly_pool_wptr_04;
.extern _dly_pool_wptr_05;
.extern _dly_pool_wptr_06;
.extern _dly_pool_wptr_07;
        .global _dly_buf_C1_DLY_18;
        .var _dly_buf_C1_DLY_18[960];  /* local fallback buffer */
        .section/dm seg_dmda;
        .global _dly_write_ptr_C1_DLY_18;
        .var _dly_write_ptr_C1_DLY_18 = 0;
        .global _dly_read_offset_C1_DLY_18;
        .var _dly_read_offset_C1_DLY_18 = 0;      /* in samples; 0 = no delay */
        .global _dly_pool_slot_C1_DLY_18;
        .var _dly_pool_slot_C1_DLY_18 = -1; /* -1 = local, 0..7 = shared long slot */
        .global _dly_local_max_C1_DLY_18;
        .var _dly_local_max_C1_DLY_18 = 960;
        .global _dly_max_C1_DLY_18;
        .var _dly_max_C1_DLY_18 = 12000;
        .global _tap_pre_fader_C1_DLY_18;
        .var _tap_pre_fader_C1_DLY_18;   /* pre-fader tap */
        .global _buf_C1_DLY_18;
        .var _buf_C1_DLY_18;

        .section/pm seg_pmco;
        .global _C1_DLY_18_process;
        _C1_DLY_18_process:

        #if DSP4_BLOCK_KERNELS
            /* ---- per-block kernel ----
             * Slot dispatch, read-offset clamp and the write-pointer
             * load/store all happen ONCE instead of 32 times. */
            r12 = dm(_dly_pool_slot_C1_DLY_18);
            i0 = _dly_buf_C1_DLY_18;
            i1 = _dly_write_ptr_C1_DLY_18;
            r3 = dm(_dly_local_max_C1_DLY_18);

            r13 = pass r12;
            if lt jump (pc, .dkb_io_C1_DLY_18);
            r14 = 8;
            comp(r12, r14);
            if ge jump (pc, .dkb_io_C1_DLY_18);
    r12 = pass r12;
    if eq jump (pc, .dkb_slot_0_C1_DLY_18);
    r14 = 1;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_1_C1_DLY_18);
    r14 = 2;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_2_C1_DLY_18);
    r14 = 3;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_3_C1_DLY_18);
    r14 = 4;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_4_C1_DLY_18);
    r14 = 5;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_5_C1_DLY_18);
    r14 = 6;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_6_C1_DLY_18);
    r14 = 7;
    comp(r12, r14);
    if eq jump (pc, .dkb_slot_7_C1_DLY_18);

        .dkb_io_C1_DLY_18:
            r2 = dm(_dly_read_offset_C1_DLY_18);
        #if DSP4_EXTRAM
        .extern _pool_backend;
        .extern _pool_phase;
        .extern _pool_line_off;
        .extern _pool_line_phase;
        .extern _pool_wstage;
        .extern _pool_rstage;
            /* THE CLAMP IS THE BACKEND DECISION, AND IT HAS TO COME FIRST.
             *
             * The line below this block clamps the requested offset into
             * the ACTIVE L2 STORAGE -- 960 samples on a channel with no
             * pool slot. Deciding head-versus-tail after that clamp would
             * be deciding it against a number that can never exceed the
             * head, so the tail would never be reached and a host asking
             * for 200 ms would silently get 20. The decision is therefore
             * taken here, on the offset the host actually wrote, and the
             * EXTRAM arm jumps over the L2 clamp entirely.
             *
             * With the backend on L2 -- no RAM found, or a product that
             * forced it off -- this falls through and the clamp below is
             * the one that ships. */
            r13 = 0;                    /* 0 = head, 1 = tail */
            r14 = dm(_pool_backend);
            r14 = pass r14;
            if eq jump (pc, .dkb_l2clamp_C1_DLY_18);
            r14 = 12000;
            comp(r2, r14);
            if lt jump (pc, .dkb_xclamped_C1_DLY_18);
            r2 = r14 - 1;               /* the FULL spec is the bound now */
        .dkb_xclamped_C1_DLY_18:
            r14 = 960;
            comp(r2, r14);
            if lt jump (pc, .dkb_ok_C1_DLY_18);
            r13 = 1;                    /* this block reads the tail */
            jump (pc, .dkb_ok_C1_DLY_18);
        .dkb_l2clamp_C1_DLY_18:
        #endif

            comp(r2, r3);
            if lt jump (pc, .dkb_ok_C1_DLY_18);
            r2 = r3 - 1;
        .dkb_ok_C1_DLY_18:
        #if DSP4_EXTRAM
            /* publish the offset the kernel will actually use -- AFTER
             * whichever clamp applied -- so the pool plans the same window
             * the loop below reads. */
            i6 = _pool_line_off;
            m6 = 17;
            modify(i6, m6);
            dm(i6, 0) = r2;
            /* the write-staging cursor: _pool_wstage + (phase*LINES + idx)
             * * BLOCK. One linear store per sample keeps the external
             * history fed without a second gather pass over the ring. */
            r8 = dm(_pool_phase);
            r9 = DSP4_POOL_LINES;
            r8 = r8 * r9 (ssi);
            r9 = 17;
            r8 = r8 + r9;
            r9 = DSP4_BLOCK_SIZE;
            r8 = r8 * r9 (ssi);
            i6 = _pool_wstage;
            m6 = r8;
            modify(i6, m6);
            l6 = 0;
        #endif

            /* CIRCULAR DAG ADDRESSING (review finding D25).
             *
             * The loop this replaces rebuilt BOTH addresses from the base
             * and a modifier on every sample -- two M-register writes, two
             * modify()s, and a compare-and-fixup to wrap the write pointer
             * by hand: seventeen instructions to move two words, on top of
             * the register-to-DAG hazard that an `m0 = r1` immediately
             * followed by a `modify(i0, m0)` costs twice per sample.
             *
             * The DAGs do modulo arithmetic for free. Give the write
             * cursor and the read cursor the same base and the same
             * length and the post-modify wraps in hardware; the two
             * cursors then keep their spacing for the whole block because
             * both advance by one. Five instructions, no M-register write
             * inside the loop, and the pointer arithmetic that is left is
             * per BLOCK.
             *
             * B AND I ARE WRITTEN TOGETHER on this core -- `b0 = x` also
             * loads i0 -- so the base goes down FIRST and the cursor after
             * it, or the offset is lost.
             *
             * Bit-exact by construction: sample k is written at
             * base + (wptr + k) mod max and read from
             * base + (wptr - offset + k) mod max, which is what the hand
             * arithmetic computed, and the write pointer handed back is
             * the same (wptr + BLOCK) mod max. */
            r7 = i0;                    /* delay-line base */
            r1 = dm(i1, 0);             /* write pointer, an OFFSET */
            r5 = r1 - r2;
            if lt r5 = r5 + r3;         /* read index, wrapped */

            m0 = 1;
            l0 = r3;
            b0 = r7;                    /* sets i0 too -- cursor next */
            r6 = r7 + r1;
            i0 = r6;                    /* write cursor */
            m2 = 1;
        #if DSP4_EXTRAM
            r13 = pass r13;
            if eq jump (pc, .dkb_rdl2_C1_DLY_18);
            /* TAIL: i2 -> _pool_rstage + ((idx*2 + phase) * 2*BLOCK)
             *              + this line's staged phase. L2 = 0: linear. */
            r8 = dm(_pool_phase);
            r9 = 34;
            r8 = r8 + r9;
            r9 = 2 * DSP4_BLOCK_SIZE;
            r8 = r8 * r9 (ssi);
            i2 = _pool_line_phase;
            m2 = 17;
            modify(i2, m2);
            r9 = dm(i2, 0);
            r8 = r8 + r9;
            l2 = 0;
            i2 = _pool_rstage;
            m2 = r8;
            modify(i2, m2);
            m2 = 1;
            jump (pc, .dkb_rdok_C1_DLY_18);
        .dkb_rdl2_C1_DLY_18:
        #endif

            l2 = r3;
            b2 = r7;
            r6 = r7 + r5;
            i2 = r6;                    /* read cursor  */
        #if DSP4_EXTRAM
        .dkb_rdok_C1_DLY_18:
        #endif


            l3 = 0;
            l4 = 0;
            l5 = 0;
            i3 = BLK_CHAIN_A;
            i4 = BLK_CHAIN_B;
            i5 = BLK_TAP_PREFDR;

        #if DSP4_DLY_SPLIT
            /* SPLIT INTO TWO PASSES (2026-09-03, review finding D25's
             * remainder). The interleaved loop below alternates a WRITE to
             * the delay line with a READ from a different address in it,
             * and the delay lines live in L2: session 3 measured that only
             * 8.4 of DLY's 63 cycles/sample was address arithmetic and
             * about 55 was that memory, and a per-sample write-then-read
             * turnaround is the shape that costs it.
             *
             * BIT-EXACT, and the reason is worth writing down rather than
             * asserting. Sample k writes index (w + k) and reads index
             * (w - off + k). A read that lands inside THIS block's writes
             * is one with k >= off, and the sample it wants was written at
             * j = k - off, which is strictly earlier than k -- so it has
             * already been written in the interleaved order too, and
             * writing the whole block first cannot change what any read
             * sees. Reads with k < off see the previous block, untouched
             * either way. off = 0 is the same argument with j = k, where
             * the interleaved loop also writes before it reads.
             *
             * Both cursors still advance by one per sample with the same
             * L, so the DAG wrap and the write pointer handed back are
             * unchanged. */
            lcntr = DSP4_BLOCK_SIZE, do .dkb_wr_C1_DLY_18 until lce;
                r0 = dm(i3, 1);
        #if DSP4_EXTRAM
                dm(i6, 1) = r0;         /* write-behind staging */
        #endif
            .dkb_wr_C1_DLY_18: dm(i0, m0) = r0;

            lcntr = DSP4_BLOCK_HALF, do .dkb_rd_C1_DLY_18 until lce;
                r0 = dm(i2, m2);        /* read;  the DAG wraps it */
                r6 = dm(i2, m2);
                dm(i5, 1) = r0;         /* pre-fader tap */
                dm(i4, 1) = r0;
                dm(i5, 1) = r6;
            .dkb_rd_C1_DLY_18: dm(i4, 1) = r6;
        #else
            lcntr = DSP4_BLOCK_SIZE, do .dkb_lp_C1_DLY_18 until lce;
                r0 = dm(i3, 1);
                dm(i0, m0) = r0;        /* write; the DAG wraps it */
        #if DSP4_EXTRAM
                dm(i6, 1) = r0;         /* write-behind staging */
        #endif

        #if DSP4_DLY_NOMEM
                r0 = pass r0;           /* MEASUREMENT ARM: no L2 read */
        #else
                r0 = dm(i2, m2);        /* read;  the DAG wraps it */
        #endif
                dm(i5, 1) = r0;         /* pre-fader tap */
            .dkb_lp_C1_DLY_18: dm(i4, 1) = r0;
        #endif

            /* the cursor back to an offset, and the DAGs back to LINEAR --
             * a non-zero L left behind would silently make the next node's
             * i0/i2 walk wrap into this delay line. */
            r1 = i0;
            r1 = r1 - r7;
            dm(i1, 0) = r1;
            l0 = 0;
            l2 = 0;
        #if DSP4_EXTRAM
            l6 = 0;
        #endif

            rts;

.dkb_slot_0_C1_DLY_18:
    i0 = _dly_pool_buf_00;
    i1 = _dly_pool_wptr_00;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_1_C1_DLY_18:
    i0 = _dly_pool_buf_01;
    i1 = _dly_pool_wptr_01;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_2_C1_DLY_18:
    i0 = _dly_pool_buf_02;
    i1 = _dly_pool_wptr_02;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_3_C1_DLY_18:
    i0 = _dly_pool_buf_03;
    i1 = _dly_pool_wptr_03;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_4_C1_DLY_18:
    i0 = _dly_pool_buf_04;
    i1 = _dly_pool_wptr_04;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_5_C1_DLY_18:
    i0 = _dly_pool_buf_05;
    i1 = _dly_pool_wptr_05;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_6_C1_DLY_18:
    i0 = _dly_pool_buf_06;
    i1 = _dly_pool_wptr_06;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
.dkb_slot_7_C1_DLY_18:
    i0 = _dly_pool_buf_07;
    i1 = _dly_pool_wptr_07;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dkb_io_C1_DLY_18);
        #endif

        /* PER-SAMPLE BODY, gated out of block-kernel builds (2026-08-29,
         * program-memory reclamation). Unlike GATE/COMP/EQ/FILT/TUBE, the
         * DLY block kernel has NO fallback into this path -- it handles
         * every slot, offset and wrap case itself and rts's -- so under
         * DSP4_BLOCK_KERNELS this body was unreachable code the linker
         * still had to place: ~212 bytes in each of 32 nodes per chip. */
        #if !DSP4_BLOCK_KERNELS
            r0 = dm(_buf_C1_TUBE_18);

            /* Default to the local short buffer. Valid slot numbers promote to a shared long buffer. */
            r12 = dm(_dly_pool_slot_C1_DLY_18);
            i0 = _dly_buf_C1_DLY_18;
            i1 = _dly_write_ptr_C1_DLY_18;
            r3 = dm(_dly_local_max_C1_DLY_18);

            r13 = pass r12;
            if lt jump (pc, .dly_io_C1_DLY_18);
            r14 = 8;
            comp(r12, r14);
            if ge jump (pc, .dly_io_C1_DLY_18);
                r12 = pass r12;
    if eq jump (pc, .dly_slot_0_C1_DLY_18);
    r14 = 1;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_1_C1_DLY_18);
    r14 = 2;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_2_C1_DLY_18);
    r14 = 3;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_3_C1_DLY_18);
    r14 = 4;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_4_C1_DLY_18);
    r14 = 5;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_5_C1_DLY_18);
    r14 = 6;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_6_C1_DLY_18);
    r14 = 7;
    comp(r12, r14);
    if eq jump (pc, .dly_slot_7_C1_DLY_18);

        .dly_io_C1_DLY_18:
            /* Clamp requested offset to the active buffer length. */
            r2 = dm(_dly_read_offset_C1_DLY_18);
            comp(r2, r3);
            if lt jump (pc, .dly_offset_ok_C1_DLY_18);
            r2 = r3 - 1;
        .dly_offset_ok_C1_DLY_18:

            /* Write at the selected write pointer. */
            r1 = dm(i1, 0);
            m0 = r1;
            modify(i0, m0);
            dm(i0, 0) = r0;

            /* Read from (write_ptr - read_offset) with wrap. */
            r5 = r1 - r2;
            if lt r5 = r5 + r3;
            r6 = r5 - r1;
            m0 = r6;
            modify(i0, m0);
            r0 = dm(i0, 0);

            /* Advance write pointer with wrap on the active storage. */
            r15 = 1;
            r1 = r1 + r15;
            comp(r1, r3);
            if ge r1 = r1 - r3;
            dm(i1, 0) = r1;

            dm(_tap_pre_fader_C1_DLY_18) = r0;
            dm(_buf_C1_DLY_18) = r0;
            rts;

        .dly_slot_0_C1_DLY_18:
    i0 = _dly_pool_buf_00;
    i1 = _dly_pool_wptr_00;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_1_C1_DLY_18:
    i0 = _dly_pool_buf_01;
    i1 = _dly_pool_wptr_01;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_2_C1_DLY_18:
    i0 = _dly_pool_buf_02;
    i1 = _dly_pool_wptr_02;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_3_C1_DLY_18:
    i0 = _dly_pool_buf_03;
    i1 = _dly_pool_wptr_03;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_4_C1_DLY_18:
    i0 = _dly_pool_buf_04;
    i1 = _dly_pool_wptr_04;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_5_C1_DLY_18:
    i0 = _dly_pool_buf_05;
    i1 = _dly_pool_wptr_05;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_6_C1_DLY_18:
    i0 = _dly_pool_buf_06;
    i1 = _dly_pool_wptr_06;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
.dly_slot_7_C1_DLY_18:
    i0 = _dly_pool_buf_07;
    i1 = _dly_pool_wptr_07;
    r3 = dm(_dly_max_C1_DLY_18);
    jump (pc, .dly_io_C1_DLY_18);
        #endif
        _C1_DLY_18_process.end:
