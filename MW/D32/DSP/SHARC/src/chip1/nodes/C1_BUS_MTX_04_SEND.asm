/*----------------------------------------------------------------------
 * Matrix 4 Bus Send (INTERCHIP_SEND)
 * Node ID:    C1_BUS_MTX_04_SEND
 * Chip:       1
 * Channels:   1
 * SPI Page:   -1
 * SPI Addr:   -1
...
/* INTERCHIP_SEND: mix-fabric line 2 slot 8 (global slot 40, signal BUS_MTX_04) */
...
.global _tx_slot_C1_BUS_MTX_04_SEND;
.var _tx_slot_C1_BUS_MTX_04_SEND[DSP4_BLOCK_SIZE];
#else
.global _tx_slot_C1_BUS_MTX_04_SEND;
.var _tx_slot_C1_BUS_MTX_04_SEND;
#endif

.section/pm seg_pmco;
.global _C1_BUS_MTX_04_SEND_process;
_C1_BUS_MTX_04_SEND_process:
#if DSP4_BLOCK_KERNELS
    /* NOTHING TO DO (review finding D25). This body used to copy the
     * source bus block into _tx_slot_C1_BUS_MTX_04_SEND so that
     * _gather_chip1 had a named array to read. The gather walks a
     * pointer table, and gen_block_io now points that table straight
     * at _buf_C1_BUS_MTX_04, so the copy was moving a block to
    ...
    rts;
#else
    r0 = dm(_buf_C1_BUS_MTX_04);
    dm(_tx_slot_C1_BUS_MTX_04_SEND) = r0;
    rts;
#endif
_C1_BUS_MTX_04_SEND_process.end:
