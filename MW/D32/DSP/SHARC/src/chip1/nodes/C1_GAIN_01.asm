 ...
 * C1_MTR_01. The block path never stores it -- it hands
 * the finished accumulators over instead. */
.global _mtr_wide_C1_GAIN_01;
.var _mtr_wide_C1_GAIN_01;
...
.cvt_C1_GAIN_01:
    ...
    call _gsimd_enter;
    lcntr = DSP4_BLOCK_HALF, do .gk_lp_C1_GAIN_01 until lce;
...
.gk_lp_C1_GAIN_01:
     ...
     * put the PEx copy here and the PEy copy in _buf_C1_GAIN_01. */
    modify(i1, -1);
    r0 = dm(i1, 0);
    dm(_tap_post_trim_C1_GAIN_01) = r0;   /* linkage scalars */
     ...
     * on stale flags. */
    r5 = DSP4_BLOCK_HALF;
    lcntr = r5; do .gk_lp_C1_GAIN_01 until lce;
        r0 = dm(i0, 1);                   /* xA */
        r3 = dm(i0, 1);                   /* xB */
        mrf = r0 * r1 (ssi);
        mrb = r3 * r1 (ssi);
...
.gk_lp_C1_GAIN_01:
        dm(i4, 1) = r3;
    dm(_tap_post_trim_C1_GAIN_01) = r3;   /* linkage scalars */
    dm(_buf_C1_GAIN_01) = r3;
    rts;
#else
    r5 = DSP4_BLOCK_SIZE;
    lcntr = r5; do .gk_lp_C1_GAIN_01 until lce;
...
.gk_lp_C1_GAIN_01:
     ...
     * mic-pre tap the D20 ruling keeps GAIN a separate node for. */
    i4 = BLK_TAP_TRIM_P1;
    r0 = _mtr_acc_C1_MTR_01;
    call _gsimd_gain_blk;
    /* The linkage scalars carry the LAST sample of the block, and
     * PEy computed it. Read it back out of the block rather than
     * out of a register: a direct store with PEYEN still up would
     * have put the PEx copy here and the PEy copy in _buf_C1_GAIN_01.
