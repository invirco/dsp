/*----------------------------------------------------------------------
 * Grp 2 EQ (EQ_BIQUAD)
 * Node ID:    C2_GRP_EQ_02
 * Chip:       2
 * Channels:   1
 * SPI Page:   1
 * SPI Addr:   1108
 * RampProfile: EqSafe
 *
 * AUTO-GENERATED by tools/dsp/dsp_codegen.py — do not edit directly.
 *----------------------------------------------------------------------*/
#include "dsp_block.h"

/* RampProfile: EqSafe | Mode: LinearFrames | Up: 12ms (72f) Down: 12ms (72f) | Curve: Linear | Scope: CoeffSetAtomic */

/* EQ_BIQUAD (FIXED Q4.28, D5): 4-band, dual-instance crossfade */
#include "blk_pool.h" 
/* SPI page=1 addr=1108 */
/* Normative model: tools/dsp/fixed_ref.py::biquad (offset form). */

.section/dm seg_dmda;
.extern _buf_C2_GRP_FDR_02;
.extern _sample_idx;

#if DSP4_BQ_FLOAT
.global _eq_coeffs_A_C2_GRP_EQ_02;
.var _eq_coeffs_A_C2_GRP_EQ_02[20] = 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000;
#elif DSP4_BQ_GUARD
.global _eq_coeffs_A_C2_GRP_EQ_02;
.var _eq_coeffs_A_C2_GRP_EQ_02[20 + 1] = 0, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000;
#else
.global _eq_coeffs_A_C2_GRP_EQ_02;
.var _eq_coeffs_A_C2_GRP_EQ_02[20] = 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000;
#endif
.global _eq_state_A_C2_GRP_EQ_02;
.var _eq_state_A_C2_GRP_EQ_02[24];
#if DSP4_BQ_FLOAT
.global _eq_coeffs_B_C2_GRP_EQ_02;
.var _eq_coeffs_B_C2_GRP_EQ_02[20] = 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000;
#elif DSP4_BQ_GUARD
.global _eq_coeffs_B_C2_GRP_EQ_02;
.var _eq_coeffs_B_C2_GRP_EQ_02[20 + 1] = 0, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000;
#else
.global _eq_coeffs_B_C2_GRP_EQ_02;
.var _eq_coeffs_B_C2_GRP_EQ_02[20] = 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000, 0x10000000, 0x10000000, 0xF0000000, 0x20000000, 0x10000000;
#endif
.global _eq_state_B_C2_GRP_EQ_02;
.var _eq_state_B_C2_GRP_EQ_02[24];

/* SPI staging -- the WIRE. Direct-form RBJ float32 under the
 * fixed arm, D5's OFFSET encoding as float32 under the float
 * arm, which stores the wire word unchanged. */
#if DSP4_BQ_FLOAT
.global _eq_coeffs_next_C2_GRP_EQ_02;
.var _eq_coeffs_next_C2_GRP_EQ_02[20] = 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000, 0x3F800000, 0x40000000, 0xBF800000, 0x40000000, 0x3F800000;
#else
.global _eq_coeffs_next_C2_GRP_EQ_02;
.var _eq_coeffs_next_C2_GRP_EQ_02[20];
#endif
.global _eq_swap_pending_C2_GRP_EQ_02;
.var _eq_swap_pending_C2_GRP_EQ_02 = 0;
#if DSP4_BQ_GUARD
.global _eq_hrw_C2_GRP_EQ_02;
.var _eq_hrw_C2_GRP_EQ_02 = 0;        /* 0 idle, 1 asked, 2 re-ask */
.global _eq_hrl_C2_GRP_EQ_02;
.var _eq_hrl_C2_GRP_EQ_02[2];       /* (block, stages) pairs */
#endif

.global _eq_active_C2_GRP_EQ_02;
.var _eq_active_C2_GRP_EQ_02 = 0;            /* 0 = A active, 1 = B */
.global _eq_xfade_alpha_C2_GRP_EQ_02;
.var _eq_xfade_alpha_C2_GRP_EQ_02 = 0.0;     /* float control */
.global _eq_xfade_step_C2_GRP_EQ_02;
.var _eq_xfade_step_C2_GRP_EQ_02 = 0.0;

.global _tap_post_eq_C2_GRP_EQ_02;
.var _tap_post_eq_C2_GRP_EQ_02;              /* Q4.28 post-EQ tap */
.global _buf_C2_GRP_EQ_02;
.var _buf_C2_GRP_EQ_02;

        #if DSP4_BLOCK_KERNELS
        .extern _blk_C2_GRP_FDR_02;
        #endif
        #if DSP4_BLOCK_KERNELS
        .global _blk_C2_GRP_EQ_02;
        .var _blk_C2_GRP_EQ_02[DSP4_BLOCK_SIZE];
        .global _bw_k_C2_GRP_EQ_02;
        .var _bw_k_C2_GRP_EQ_02;        /* the caller's _sample_idx */
        .global _bw_s0_C2_GRP_EQ_02;
        .var _bw_s0_C2_GRP_EQ_02;       /* walking source pointer */
        .global _bw_d0_C2_GRP_EQ_02;
        .var _bw_d0_C2_GRP_EQ_02;       /* walking sink pointer */
        #endif

.section/pm seg_pmco;
.extern _bq_fx_cascade_N;
#if DSP4_BLOCK_KERNELS
.extern _bq_fx_cascade_blk;
#endif
.extern _bq_fx_convert_N;
#if DSP4_BQ_GUARD
.extern _bq_hr_node1;
#endif
.global _C2_GRP_EQ_02_process;
_C2_GRP_EQ_02_process:
        #if DSP4_BLOCK_KERNELS
    /* ---- chip-2 per-block steady state (review finding D16) ----
     *
     * The same fusion chip 1's EQ has had since the pool rewrite,
     * on chip 2's own buffers: one call to _bq_fx_cascade_blk walks
     * all 4 stages over the whole block with the state and the
     * error feedback held in registers, instead of BLOCK calls to
     * _bq_fx_cascade_N that tear the 80-bit accumulator apart and
     * put it back every sample.
     *
     * _bq_fx_cascade_blk was proved bit-exact against
     * _bq_fx_cascade_N on the part (DSP4_BQ_SELFTEST, 0 of 64
     * samples differing, two stages with DIFFERENT coefficients,
     * across a block boundary), so what is left to get right is this
     * wrapper -- and the transient path below is the per-sample
     * reference body itself, one sample at a time, so the alpha
     * bookkeeping and a crossfade COMPLETING mid-block are right by
     * construction rather than by re-derivation. A crossfade lasts
     * 576 samples and is a transient; its cost does not matter. */
    r4 = dm(_eq_swap_pending_C2_GRP_EQ_02);
    r5 = dm(_eq_xfade_step_C2_GRP_EQ_02);
    r4 = r4 or r5;
#if DSP4_BQ_GUARD
    r5 = dm(_eq_hrw_C2_GRP_EQ_02);
    r4 = r4 or r5;      /* a sizing in flight is a transient too */
#endif
    r4 = pass r4;
    if ne jump (pc, .eqkb_tr_C2_GRP_EQ_02);

    l0 = 0;
    l1 = 0;
    l2 = 0;
    l3 = 0;
    l4 = 0;
    /* Copy in, then cascade IN PLACE. Chip 1 skips the copy because
     * its pool slot IS the previous node's output and the strip owns
     * it end to end; on chip 2 the input block belongs to the
     * producer and can have more than one reader -- a meter, a tap,
     * the main mix bus reading seventeen -- so filtering it where it
     * stands would corrupt every other reader. Two memory operations
     * per sample against a 4-stage cascade. */
    i3 = _blk_C2_GRP_FDR_02;
    i4 = _blk_C2_GRP_EQ_02;
    lcntr = DSP4_BLOCK_SIZE, do .eqkb_cp_C2_GRP_EQ_02 until lce;
        r0 = dm(i3, 1);
    .eqkb_cp_C2_GRP_EQ_02: dm(i4, 1) = r0;

    r4 = dm(_eq_active_C2_GRP_EQ_02);
    r4 = pass r4;
    if ne jump (pc, .eqkb_b_C2_GRP_EQ_02);
    i0 = _eq_coeffs_A_C2_GRP_EQ_02;
    i1 = _eq_state_A_C2_GRP_EQ_02;
    jump (pc, .eqkb_go_C2_GRP_EQ_02);
.eqkb_b_C2_GRP_EQ_02:
    i0 = _eq_coeffs_B_C2_GRP_EQ_02;
    i1 = _eq_state_B_C2_GRP_EQ_02;
.eqkb_go_C2_GRP_EQ_02:
    i2 = _blk_C2_GRP_EQ_02;
    r4 = 4;
    call _bq_fx_cascade_blk;
    /* The scalar the per-sample build publishes, kept live off the
     * LAST sample of the block. Nothing under block kernels reads it,
     * but a host peek at this node must not report a word from
     * whenever the build last ran per-sample. */
    l4 = 0;
    m4 = DSP4_BLOCK_SIZE-1;
    i4 = _blk_C2_GRP_EQ_02;
    modify(i4, m4);
    r0 = dm(i4, 0);
    dm(_tap_post_eq_C2_GRP_EQ_02) = r0;
    dm(_buf_C2_GRP_EQ_02) = r0;
    rts;

.eqkb_tr_C2_GRP_EQ_02:
#endif
        #if DSP4_BLOCK_KERNELS
            /* ---- generic per-block wrapper (review finding D16) ----
             * Runs the per-sample reference body BLOCK times over this
             * node's own block buffer, staging each sample through the
             * scalar _buf_ words the body already reads and writes. Same
             * arithmetic, same order, same result as the per-sample
             * build; what it removes is the chain's call/rts and the
             * _sample_idx guard being re-evaluated from the chain.
             *
             * SAMPLE 0 IS PEELED AND _sample_idx IS NOT COUNTED
             * (2026-09-02). The loop used to carry a DM sample index,
             * loading it, storing it to _sample_idx, reloading it,
             * incrementing it and storing it back -- five instructions
             * a sample to drive a guard that only ever asks whether the
             * index is ZERO. Every one of the 119 wrapped bodies was
             * checked: all of them compare it against 0 and none reads
             * it for anything else. So sample 0 runs once outside the
             * loop with _sample_idx = 0, the word is then set to 1, and
             * the remaining BLOCK-1 samples run with the guard shut.
             * The block-rate conversions still fire exactly once per
             * block, which is the property the guard exists for.
             *
             * NOT a free win to generalise: the C2_RECV_* stimulus
             * nodes read _sample_idx & 1 to alternate the profile
             * square, and they would break under this. They are not
             * wrapped nodes -- they carry their own block form -- and
             * that is why the audit is per-node and recorded here.
             * 38 cycles a block per wrapped node.
             */
            i4 = _blk_C2_GRP_FDR_02;
            r3 = i4;
            dm(_bw_s0_C2_GRP_EQ_02) = r3;
            i4 = _blk_C2_GRP_EQ_02;
            r3 = i4;
            dm(_bw_d0_C2_GRP_EQ_02) = r3;
            r5 = dm(_sample_idx);
            dm(_bw_k_C2_GRP_EQ_02) = r5;
            r5 = 0;
            dm(_sample_idx) = r5;
            /* sample 0, peeled: the block-rate guard fires here */
                r3 = dm(_bw_s0_C2_GRP_EQ_02);
                i4 = r3;
                r0 = dm(i4, 0);
                dm(_buf_C2_GRP_FDR_02) = r0;
                r3 = r3 + 1;
                dm(_bw_s0_C2_GRP_EQ_02) = r3;
                call _C2_GRP_EQ_02_process_sample;
                r0 = dm(_buf_C2_GRP_EQ_02);
                r3 = dm(_bw_d0_C2_GRP_EQ_02);
                i4 = r3;
                dm(i4, 0) = r0;
                r3 = r3 + 1;
                dm(_bw_d0_C2_GRP_EQ_02) = r3;
            r5 = 1;
            dm(_sample_idx) = r5;   /* guard shut for 1..BLOCK-1 */
            lcntr = DSP4_BLOCK_SIZE-1, do .bwlp_C2_GRP_EQ_02 until lce;
                r3 = dm(_bw_s0_C2_GRP_EQ_02);
                i4 = r3;
                r0 = dm(i4, 0);
                dm(_buf_C2_GRP_FDR_02) = r0;
                r3 = r3 + 1;
                dm(_bw_s0_C2_GRP_EQ_02) = r3;
                call _C2_GRP_EQ_02_process_sample;
                r0 = dm(_buf_C2_GRP_EQ_02);
                r3 = dm(_bw_d0_C2_GRP_EQ_02);
                i4 = r3;
                dm(i4, 0) = r0;
                r3 = r3 + 1;
            .bwlp_C2_GRP_EQ_02: dm(_bw_d0_C2_GRP_EQ_02) = r3;
            r5 = dm(_bw_k_C2_GRP_EQ_02);
            dm(_sample_idx) = r5;
            rts;

        .global _C2_GRP_EQ_02_process_sample;
        _C2_GRP_EQ_02_process_sample:
        #endif


    /* new coefficients staged? */
    r4 = dm(_eq_swap_pending_C2_GRP_EQ_02);
    #if DSP4_BQ_GUARD
    r5 = dm(_eq_hrw_C2_GRP_EQ_02);
    r4 = r4 or r5;
    #endif
    r4 = pass r4;
    if ne call _eq_start_xfade_C2_GRP_EQ_02;

    /* crossfading? (float 0.0 is all-zero bits) */
    r4 = dm(_eq_xfade_step_C2_GRP_EQ_02);
    r4 = pass r4;
    if ne jump (pc, .eq_xfade_C2_GRP_EQ_02);

    /* ===== steady state: active instance only ===== */
    r0 = dm(_buf_C2_GRP_FDR_02);
    r4 = dm(_eq_active_C2_GRP_EQ_02);
    r4 = pass r4;
    if ne jump (pc, .eq_ss_b_C2_GRP_EQ_02);
    i0 = _eq_coeffs_A_C2_GRP_EQ_02;
    i1 = _eq_state_A_C2_GRP_EQ_02;
    jump (pc, .eq_ss_go_C2_GRP_EQ_02);
.eq_ss_b_C2_GRP_EQ_02:
    i0 = _eq_coeffs_B_C2_GRP_EQ_02;
    i1 = _eq_state_B_C2_GRP_EQ_02;
.eq_ss_go_C2_GRP_EQ_02:
    r4 = 4;
    call _bq_fx_cascade_N;
    dm(_tap_post_eq_C2_GRP_EQ_02) = r0;
    dm(_buf_C2_GRP_EQ_02) = r0;
    rts;

    /* ===== crossfade: run both, blend fixed ===== */
.eq_xfade_C2_GRP_EQ_02:
    r0 = dm(_buf_C2_GRP_FDR_02);
    r13 = r0;                     /* input (r13-r15 preserved by lib) */
    i0 = _eq_coeffs_A_C2_GRP_EQ_02;
    i1 = _eq_state_A_C2_GRP_EQ_02;
    r4 = 4;
    call _bq_fx_cascade_N;
    r14 = r0;                     /* ya */
    r0 = r13;
    i0 = _eq_coeffs_B_C2_GRP_EQ_02;
    i1 = _eq_state_B_C2_GRP_EQ_02;
    r4 = 4;
    call _bq_fx_cascade_N;        /* r0 = yb */

    /* orient: out = old + alpha*(new - old); dormant is new */
    r4 = dm(_eq_active_C2_GRP_EQ_02);
    r4 = pass r4;
    if eq jump (pc, .eq_bl_C2_GRP_EQ_02);     /* active A -> new is B (r0) */
    r5 = r14;                      /* new = ya */
    r14 = r0;                      /* old = yb */
    r0 = r5;
.eq_bl_C2_GRP_EQ_02:
    f4 = dm(_eq_xfade_alpha_C2_GRP_EQ_02);
    r5 = 0x4F000000;               /* 2^31 as float */
    f5 = r5;
    f4 = f4 * f5;
    r4 = fix f4;                   /* alpha_q31; `fix` saturates */
    /* alpha*(new - old) as TWO MACs into the 80-bit MRF, so the
     * difference is NEVER formed in a 32-bit register (review
     * finding D3). `new` and `old` are independently saturated
     * Q4.28 outputs, so new-old spans +/-(2^32-1) and the old
     * `r5 = r0 - r14` wrapped when the two instances straddled
     * full scale mid-swap -- up to a block of full-scale-wrong
     * samples, a click. Same instruction count, and identical
     * arithmetic everywhere the subtract did not wrap.
     * Model: fixed_ref.xfade_blend. The final add cannot
     * overflow -- the result is a convex combination of two
     * int32s, bounded by them; the bound is in numeric-spec.md.
     *
     * EMITTED FROM ONE PLACE: _xfade_blend_core() in
     * dsp_codegen.py. Every EQ/GEQ/AFB/FILT/CROSSOVER node and
     * the in-part self-test (lib/num_selftest.asm) get these
     * exact instructions from this one expression, so the
     * sequence the self-test proves bit-exact against the model
     * is the sequence the nodes run. */
    mrf = r0 * r4 (ssi);           /* + new*alpha */
    mrf = mrf - r14 * r4 (ssi);    /* - old*alpha */
    r5 = 0x40000000;               /* 2^30 rounding half */
    r12 = 1;
    mrf = mrf + r5 * r12 (ssi);
    r5 = mr0f;
    r12 = mr1f;
    r5 = lshift r5 by -31;
    r12 = lshift r12 by 1;
    r5 = r5 or r12;
    r0 = r14 + r5;                 /* blended output */
    dm(_tap_post_eq_C2_GRP_EQ_02) = r0;
    dm(_buf_C2_GRP_EQ_02) = r0;

    /* advance alpha (float control) */
    f4 = dm(_eq_xfade_alpha_C2_GRP_EQ_02);
    f5 = dm(_eq_xfade_step_C2_GRP_EQ_02);
    f4 = f4 + f5;
    dm(_eq_xfade_alpha_C2_GRP_EQ_02) = f4;
    r5 = 0x3F800000;               /* 1.0f */
    f5 = r5;
    comp(f4, f5);
    if lt rts;
    /* crossfade done: dormant becomes active */
    r4 = dm(_eq_active_C2_GRP_EQ_02);
    r5 = 1;
    r4 = r4 xor r5;
    dm(_eq_active_C2_GRP_EQ_02) = r4;
    r4 = 0;
    dm(_eq_xfade_step_C2_GRP_EQ_02) = r4;
    dm(_eq_xfade_alpha_C2_GRP_EQ_02) = r4;
    rts;

    /* ===== stage new coeffs into the dormant instance ===== */
_eq_start_xfade_C2_GRP_EQ_02:
    #if DSP4_BQ_GUARD
    /* Convert, size, and hold the fade until H is written. */
    r0 = _eq_hrw_C2_GRP_EQ_02;
    r1 = _eq_hrl_C2_GRP_EQ_02;
    r2 = _eq_active_C2_GRP_EQ_02;
    r3 = _eq_coeffs_A_C2_GRP_EQ_02;
    r4 = _eq_coeffs_B_C2_GRP_EQ_02;
    r5 = _eq_state_A_C2_GRP_EQ_02;
    r6 = _eq_state_B_C2_GRP_EQ_02;
    r7 = _eq_coeffs_next_C2_GRP_EQ_02;
    r8 = 4;
    call _bq_hr_node1;      /* i2 = dormant state on success */
    r0 = pass r0;
    if eq rts;              /* not sized yet; back next block */
    r4 = 0;
    dm(_eq_swap_pending_C2_GRP_EQ_02) = r4;
    #endif
#if !DSP4_BQ_GUARD
    r4 = 0;
    dm(_eq_swap_pending_C2_GRP_EQ_02) = r4;
    i0 = _eq_coeffs_next_C2_GRP_EQ_02;    /* float staged */
    r4 = dm(_eq_active_C2_GRP_EQ_02);
    r4 = pass r4;
    if ne jump (pc, .eq_st_a_C2_GRP_EQ_02);
    i1 = _eq_coeffs_B_C2_GRP_EQ_02;       /* dormant = B */
    i2 = _eq_state_B_C2_GRP_EQ_02;
    jump (pc, .eq_st_go_C2_GRP_EQ_02);
.eq_st_a_C2_GRP_EQ_02:
    i1 = _eq_coeffs_A_C2_GRP_EQ_02;
    i2 = _eq_state_A_C2_GRP_EQ_02;
.eq_st_go_C2_GRP_EQ_02:
    r4 = 4;
    call _bq_fx_convert_N;
#endif
    /* zero dormant state (24 words) */
    r4 = 0;
    r5 = 24;
    lcntr = r5, do .eq_zst_C2_GRP_EQ_02 until lce;
.eq_zst_C2_GRP_EQ_02:
        dm(i2, 1) = r4;
    /* start ramp: step = 1/XFADE_SAMPLES (float control) */
    f0 = 0.001736111111111111;
    dm(_eq_xfade_step_C2_GRP_EQ_02) = f0;
    r4 = 0;
    dm(_eq_xfade_alpha_C2_GRP_EQ_02) = r4;
    rts;
_C2_GRP_EQ_02_process.end:
