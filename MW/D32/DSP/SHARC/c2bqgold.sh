...
run_arm() {   # $1 = tag, $2 = C2_BQ_GRAPH, $3 = C2_BQ_NEGCTL, $4 = C2_BQ_NOLATCH
  ...
  ssh $BENCH "STAGE='$STAGE' bash /home/app/c2gold_run.sh $DWELL $STAGE/bq$P.json $BLOCK" \
    2>&1 | sed "s/^/  arm$P: /"
}
...
scp -q $BENCH:$STAGE/bq0.json $BENCH:$STAGE/bq1.json \
       $BENCH:$STAGE/bqn.json $BENCH:$STAGE/bqr.json \
       "$WORK/"
