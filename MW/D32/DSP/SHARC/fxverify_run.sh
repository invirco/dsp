#!/bin/bash
# fxverify_run.sh — bench half of fxverify.sh.
#
...
for cycle in 1 2 3; do
  ...
  ARGS="--landed landed-$PRODUCT.json --n ${N:-1024} --json ${OUT:-fxverify.json}"
  ARGS="$ARGS --node ${NODE:-C2_FX_ENG_01} --prefix ${PREFIX:-Fx001}"
  ARGS="$ARGS --upstream ${UPSTREAM:-_buf_C2_RECV_FX_01}"
  ARGS="$ARGS --inject ${INJECT:-_rx_ic_slot_C2_RECV_FX_01}"
  python3 dsp4_fx_verify.py $ARGS
  exit $?
done
