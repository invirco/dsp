#!/bin/bash
# s25pan_run.sh — bench half of s25pan.sh (see that file).
#
# Same boot/config retry ladder as conform_run.sh. The S25 pan-law gate is
# CHIP 1's -- the table, the index, the law select and the three legs --
# but BOTH chips are still configured, because a strip whose downstream
# never came up is a strip whose control epoch and ramps are in an
# unknown state. A verdict taken through a half-configured graph is
# fiction.
set -u
OUT_N="${1:-1}"
PRODUCT="${PRODUCT:-d24}"
...
for attempt in 1 2 3 4 5; do
  ...
  for g in 1 2 3 4; do
    python3 dsp4_diag.py --chip 1 >/dev/null 2>&1
    python3 dsp4_s25_pan_probe.py "$OUT_N" && exit 0
    sleep 2
  done
  echo "  (attempt $attempt: no verdict — re-booting)"
done
echo "no usable S25 pan probe run in 5 attempts"
exit 4
