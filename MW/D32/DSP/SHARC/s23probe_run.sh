#!/bin/bash
# s23probe_run.sh — bench half of s23probe.sh (see that file).
#
# Same boot/config retry ladder as conform_run.sh, and BOTH chips are
# configured because the S23 gates straddle them: gates 2 and 3 read chip
# 2's FX return, main mix and aux sums, gate 4 reads chip 1's channel
# meter. A verdict taken through a half-configured graph is fiction.
set -u
GATES="${1:-2,3,4}"
PRODUCT="${PRODUCT:-d24}"
...
for attempt in 1 2 3 4 5; do
  ...
  for g in 1 2 3 4; do
    python3 dsp4_diag.py --chip 1 >/dev/null 2>&1
    python3 dsp4_s23_probe.py "$GATES" && exit 0
    sleep 2
  done
  echo "  (attempt $attempt: no verdict — re-booting)"
done
echo "no usable S23 probe run in 5 attempts"
exit 4
