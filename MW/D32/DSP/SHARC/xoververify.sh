...
PRODUCT="${PRODUCT:-d24}"
OUT="${OUT:-xoververify-$(date +%Y%m%d-%H%M).json}"
...
if [ "${BUILD:-1}" = "1" ]; then
  ./build.sh > /tmp/xoververify_build.log 2>&1
  if [ "$(grep -ciE '\[Error|Build FAILED' /tmp/xoververify_build.log)" -ne 0 ]; then
    echo "BUILD FAILED"; grep -iE '\[Error' /tmp/xoververify_build.log | head; exit 1; fi
...
scp -q $ROOT/tools/pi/dsp4_xover_verify.py $ROOT/tools/pi/dsp4_conform.py \
    $ROOT/tools/pi/dsp4_family_verify.py $ROOT/tools/pi/dsp4_block.py \
    $ROOT/tools/dsp/fixed_ref.py $ROOT/tools/dsp/xover_ref.py \
...
scp -q xoververify_run.sh $BENCH:/home/app/ || exit 3

ssh $BENCH "STAGE='$STAGE' PRODUCT=$PRODUCT N='${N:-1024}' \
            OUT='$OUT' bash /home/app/xoververify_run.sh"
RC=$?
