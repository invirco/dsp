...
# THE SCRATCH TREE IS KEYED BY THE STATE OF EVERYTHING IT IS GENERATED
# FROM, and that is not paranoia.
#
...
# is a stale-golden defect wearing a performance hat.
#
# The first repair was "regenerate every run", marked with a $$ file. That
# is weaker than it looks: the marker is a PID, PIDs are reused, and two
# runs in the same second collide on one directory -- a stale reproduction
# stayed POSSIBLE, just unlikely. The key below makes it impossible instead
# of unlikely, which is what the bug deserves: the directory NAME carries a
# digest of the block size, dsp.csv, the codegen and every file of src/, so
# a tree generated from different inputs is a different directory and
# cannot be picked up by accident. `.srckey` is written LAST and re-checked,
# so a tree left half-generated by a killed run is regenerated rather than
# built from. Cost: one sha256 pass over src/ (~0.3 s) per run.
...
print(a('proc_cyc'), a('proc_passes'))")"
    ...
    [ "${REPS:-1}" -gt 1 ] && echo "block=$B strips=$N limit=$L  MIN=${BEST:-none} cycles/block over ${REPS} boot(s)"
done
