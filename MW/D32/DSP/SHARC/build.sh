...
# THE CUE BUS (S65, src/chip1/cue.asm + src/chip2/cue_rx.asm, generated by
# tools/dsp/dsp_codegen.py::gen_cue): the stereo cue sum on chip 1, sent to
# chip 2 on MIX_2 slots 9/10 for C2_MON (source 13). Off by default; with 0
# both images are byte-identical to a build without it.
DSP4_CUE="${DSP4_CUE:-0}"
CFLAGS="$CFLAGS -DDSP4_CUE=$DSP4_CUE"
ASMFLAGS="$ASMFLAGS -DDSP4_CUE=$DSP4_CUE"
if [ "$DSP4_CUE" != "0" ]; then
    echo "  *** CUE BUILD: DSP4_CUE=$DSP4_CUE ***"
fi
# The RTA filterbank (S64; chip 1 since S65, src/chip1/rta.asm, generated by
# tools/dsp/dsp_codegen.py::gen_rta): 31 x 1/3-octave x 3 biquads x 2
# channels + ballistics on the cue bus, run after chip 1's chain while RtaOn
# = 1. Needs DSP4_CUE=1. Off by default; with 0 the image is byte-identical.
DSP4_RTA="${DSP4_RTA:-0}"
CFLAGS="$CFLAGS -DDSP4_RTA=$DSP4_RTA"
ASMFLAGS="$ASMFLAGS -DDSP4_RTA=$DSP4_RTA"
if [ "$DSP4_RTA" != "0" ]; then
    echo "  *** RTA BUILD: DSP4_RTA=$DSP4_RTA ***"
fi
