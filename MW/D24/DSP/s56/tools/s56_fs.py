"""s56_fs.py — the 0.98 FS ceiling, digital: TEST_OSC injected into strip 20's INPUT block (after C1_IN_20, before
C1_GAIN_20), swept through and past full scale. Reads the SAME meter S54-5 read (_mtr_peak_C1_MTR_20, strip 20's
gain-stage block peak), TEST_MEAS on strip 20 post-fader, and a 4096 capture's sample peak + FFT THD+N."""
...
MP = sc.sym['_mtr_peak_C1_MTR_20']
for k in ('CompOn001', 'GateOn001', 'EqOn001', 'TubeOn001', 'Mute001'):
    assert c1.r('Chan020' + k) == 0, k
print('strip 20 gain %.4f level %.4f' % (X.from_f32(c1.r('Chan020Gain001')), X.from_f32(c1.r('Chan020Level001'))))
r.meas(20)
rows = []
...
for lin in [0.5, 0.9, 0.95, 0.979, 1.0, 1.02, 1.1, 1.259, 1.585, 2.0, 4.0]:
    r.osc(freq=1000.0, level_db=20 * math.log10(lin), on=True, chan=20)
