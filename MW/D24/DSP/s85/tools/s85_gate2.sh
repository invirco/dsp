#!/bin/bash
# s85_gate2.sh — THE TWO-SIDED READING ON THE RETIMED LANES.
#
# Same shape as s84_gate3b.sh and for the same reason: the pins and the
# buffers are read in ONE pass, on ONE bitstream, with the same rails and the
# same 595 image, so the difference between them cannot be the bitstream.
#
# What has changed is the lane path. On every bitstream this bench has carried,
# ad[0..2] crossed U3 as a plain wire and the DSP sampled the AK5558s' data
# through the round trip plus U3's pin-to-pin delay. Here they are captured on
# bck8_launch -- the edge S85 gate 1 measured the data sitting stable on -- and
# handed to the DSP as a register output with the full half bit period of
# setup, which is the arrangement every lane that works already uses.
#
# The witness is still in this bitstream and its LAUNCH bank counts exactly the
# bits the retiming register captures, same strobe, same data. So the pins side
# below is not merely "are the pins alive", it is "this is what the register
# holds".
set -u
STAGE="${STAGE:-/home/app/s83tn}"
OUTDIR=/home/app/s85
TAG="${TAG:-adrt}"
EXPECT="${EXPECT:-02c13e1a}"
mkdir -p "$OUTDIR"
cd "$STAGE"
...
flock "$BENCH_LOCK_FD"
printf 'pid=%s script=s85_gate2.sh started=%s\n' "$$" "$(date -u +%FT%TZ)" > "$BENCH_LOCKFILE.info"
...
SYMDIR=$STAGE python3 $STAGE/s83_lanescan.py --symdir $STAGE --reps 16 \
    --tag "$TAG-chain-FC-rails-up" --json "$OUTDIR/lanes-$TAG.json" 2>&1 | head -6
python3 - <<PY
import json
d = json.load(open('$OUTDIR/lanes-$TAG.json'))
...
for l in mv[:40]:
    print('   MOVING %-10s distinct %d' % (l['lane'], l['distinct']))
PY

echo
echo "=== SIDE 2: THE PINS (and what the retime register holds), same pass ==="
python3 /home/app/dsp4_ad_witness.py --reps 2 2>&1 | tee "$OUTDIR/pins-$TAG.txt" \
