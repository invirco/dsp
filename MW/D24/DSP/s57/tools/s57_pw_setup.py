"""s57_pw_setup.py — PW bench window: MIC 5 (strip 20) at code 63 -> AUX 1 alone, unity, TEST_OSC off.
Guard first: the lane at code 63 must read the 150 ohm floor (~ -92 dBFS), not the loop (~ -52): routing MIC 5 at
+58.7 dB to AUX 1 with the loop cable on J45->J25 would close a feedback loop. Then dsp4_apply_strip.py 20 1 (every
strip and USB/BT/CodecAux/Pi off AUX 1, strips muted, strip 20 unmuted, post-fader send 1.0), then strip 20 off MAIN
and every other bus, EQ/comp/gate/tube off, gain/fader unity. Verify: all AuxOn001 read back, strip-20 16k capture FFT
...
P('strip 20 post-fader capture (feeds AUX 1 at unity): DC-24k %.2f, 20-20k %.2f, A %.2f dBFS, ovr +%d' % (raw, b20, aw, cap['overruns']))
