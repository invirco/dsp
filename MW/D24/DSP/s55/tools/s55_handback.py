...
it); strip 1 AuxSend001 back to 0.0 (the J32 donor switch set it to 1.0); MeasChan = the MIC 5 strip (T.LOOP); the 595 image S57 left:
...
rec = {'ev': 'handback', 'img': ['%02X' % b for b in R.image], 'osc_on': R.rd(T.A_OSCON), 'meas': R.rd(T.A_MEASCHAN),
       ...
       's1_auxsend': X.from_f32(R.c1.r('Chan001AuxSend001')), 'mic5_strip_rms': [w['rms'] for w in m['rows']],
       'an_en': R.an_en()[1]}
T.log(rec)
