...
hash) and a 5-bit config field into the bitstream, and records both in the
manifest. They are GENERATED by `build.sh`, never typed, and they do not
feed the hash — so "read the register, compare to the manifest" is a real
check rather than a transcription.

MAX V has no configuration readback over SVF, the DSPs have no link into
this CPLD, and the TEST pins land on a DNP header, so the register is read
over the one path off the part that needs no hands — the CM4 PCM link — by
knocking on it:

```
python3 tools/pi/dsp4_logic_id.py --expect ae1ac4a9
```

The Pi plays `{L = 0xD5D51D1D, R = 0x2A2AE2E2}` (R the exact bit-inverse
of L) and LOGIC answers `{L = design_id, R = 0xD594<cfg_bits>}` for 128
frames, then returns to audio. It is built in EVERY configuration,
including shipping: a false trigger needs that specific 64-bit pair and
costs 2.7 ms of the CM4's own return stream, and touches nothing on the
DSP-facing side, the DAC lanes, the NET lanes or the panel. A bitstream
that answers nothing predates the register (2026-09-09), which is itself
...
- `quartus/` — 5M1270ZT144C4 project with the REAL pin assignments
  ...
  Current build: **404 LE / 1270 (32%), 71 pins, Fmax 68.66 MHz**
  (2026-09-09 `_maincap`; it was 156 LE / 12% before the frame-locked
  capture snapshot and the design-ID register, which cost ~248 LE
  between them and did not cost any Fmax). **That growth matters on the
  rev-D 5M570ZT144C4**, where the same RTL previously sat at 27% and
  closed at only 50.67 MHz: 404 LE is 71% of that part. Re-run the STA
  gate there before assuming it still fits. On the rev-D
...
| `sim/tb_pcm_capture.v` | The CAPTURE direction: DSPB TDM8 → Pi. A different word in every slot on every frame, so a read-out that walks the live register file cannot pass; checks L and R are the words of ONE frame at a constant lag, for both the `_maincap` slots (0/1) and the shipping return (2/3); and the design-ID knock |
...
`tb_pcm_capture.v` exists because `tb_pcm_reframe.v` leaves `tdm_in` and
`bck8_sample` **dangling** — until 2026-09-09 nothing simulated the
capture direction at all, and two defects shipped through a green sim
gate: a read-out that spliced every word out of two consecutive DSP
frames, and a period decode one BCK early that a compensating
`CAP_EXTRA_DELAY` had been hiding since August. Findings S7-1 to S7-3.
