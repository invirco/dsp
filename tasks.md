## HUB DISPATCH 2026-09-08 16:48Z — VIRTUAL AUDIO over the CPLD loop, continued: the golden harness gets its hardware target (steps 2–4 of the queued block), on the float image at defs-v2026.09.08.2 — every kernel family exercised through the real SHARC path, per-family hardware-verified lines, and the coverage fraction reported   [status: 🟢 done — **EVERY D24 FAMILY THE LANDED CONTRACT ADDRESSES IS EXERCISED ON THE PART AND 14 OF 20 PASS: 3,028 OF 3,698 ADDRESSED CELLS, 82 %. SEVEN pass on the STRICT bar — the part's own samples reproduced WORD FOR WORD by the reference model with its deliberately-wrong twin required to disagree — covering 1,828 cells (49 %).** **THE CONTRACT'S FIRST LIVE USE AND IT HELD.** `tools/dsp/landed_map.py` reads `defs/products/d24/dsp.csv` and stages it beside the images with the `defs.lock` pin and the CSV sha256; `tools/pi/dsp4_family_verify.py` resolves EVERY address it writes by CELL NAME out of that file — no stride arithmetic anywhere. All 25 constants the bench harness had transcribed out of `dsp_address_map.md` agree with the landed address for their cell, and **every landed address written ANSWERED** (`SPI_ERR_COUNT` either side of every write; no `NO_ENTRY` on any family, both chips). **A MAJOR DEFECT FOUND AND FIXED THE SAME DAY, AND THE FIX IS GENERIC.** `ChanGateHold` reached `_gate_hold_*` — an integer SAMPLE COUNT, initialiser 2400 = 50 ms — unconverted, so the documented 1.0 ms landed 0x3F800000 = 1,065,353,216 samples ≈ 6.2 hours and **a gate that had opened never closed again**. `ChanDelay` had the identical shape (250.0 ms → 0x437A0000, clamped to the buffer), which is exactly why a one-off for Hold was refused: `gen_dsp.py` now builds `_spi_dispatch_cN_convert[]` FROM the landed `defs/common/wire/wire-units.csv` and `spi_handler.asm` applies it **at the wire and in BOTH directions** — at the wire because the ramp engine interpolates between the current word and the new one, both directions because save-and-restore would otherwise read samples back and write them as ms. Verified on the part: GateHold 1.0 ms → 48, 50.0 ms → **2400** (the generator's own initialiser), Delay 20.0 ms → 960, every one reading back in ms. The nine declared-but-unconvertible rows (the four ms→alpha ones say 'needs conversion declared' in as many words) are PRINTED every generation, not skipped — a row plus a rule away, and NOT invented here. Images moved as they must: chip1 906a70f7→**a07d3865**, chip2 3a2d930c→**073d80ef**. **THE CM4 DUPLEX LOOP IS UP.** The hub's pin ruling (GPIO17 = CS6 as `sdmode-gpios`) WORKED — voicehat probed and the card came up — and voicehat is still unusable, because its DAI is 48 kHz only while `slot-map.csv` lane A_I6 fixes the Pi frame at 192 kHz (measured: ~1.6 s capture overrun, a known word returning an unrelated constant). ak4554 gave a real duplex device and is S16_LE only; dit+dir on one link comes out capture-only. **`invirco,dsp4-pcm-dummy` — forty lines of DAI declaration, both directions, 8–192 kHz, S32_LE, no registers, no control bus, NO GPIO — closes it: `00-00 … playback 1 : capture 1`, S32_LE at 192 kHz, no rate clamp, no xrun across a 96,000-word duplex run.** CS6 stays free and the ruling is recorded rather than consumed. Exact module/overlay/config.txt lines for `cm4-setup-pi.sh` are in findings S2-7; `cm4-setup-pi.sh` NOT edited, per dispatch. **STILL NOT A MEASUREMENT CHANNEL, and no longer because of the overlay**: a known word returns riding a ~0x11E7E000 DC pedestal because the main chain sums 17 sources and none of its nodes was set to bypass — that is step 1's pass-through work and the only thing between here and a latency figure, which is therefore NOT quoted. **FOUR FAMILIES ANSWER EVERY LANDED ADDRESS AND REACH NO SAMPLE — 646 cells: ANTI_FB, GEQ, CROSSOVER, FX_ENGINE**, proved with an IMPULSE (frequency-complete; a DC step cannot see a peaking section at all, which is how the first pass got GEQ/AFB/XOVER wrong) walked down the chain with 0 of 32 words differing at every node. ANTI_FB and FX_ENGINE CORROBORATE the D38 static list; **GEQ and CROSSOVER are NOT on it**, so D38's 896 under-reports by at least 372 — recorded in `docs/contract/inert-cells-d38.md`, generated table deliberately unchanged since resolving WHY the static test misses them is what would justify moving it. **FOUR INSTRUMENT FAULTS FOUND, EACH OF WHICH HAD ALREADY PRODUCED A WRONG ANSWER ABOUT THE FIRMWARE, all fixed**: BQCVT scored the FLOAT image against the FIXED reference and called EQ/FILT FAILED (part held float 1.0, model predicted Q4.28 1.0); `dsp4_node_verify.vpeek`'s zero sentinel is armed in `main()`, so a tool calling `run_node()` directly read every genuinely-ZERO parameter as unreadable and COMPRESSOR lost its verdict twice on its hard-knee words; the DC-step problem above; and **METER, which this run first reported as FAILED and is BIT-EXACT** — `mtrverify.sh` reads METER_BIT_EXACT (ms64 exact, both pk64 words exact, float readback 0.5/0.5 at 0.000e+00, BLOCK=32 control correctly rejected). The 7.05 readback was a peak HOLD still carrying the contract sweep that ran before it; 7.05 just under the Q8.24 ceiling of 8.0 was the tell and it was not followed. `hw_coverage.py` now scores a family by a DEDICATED bar's verdict where one has been run, recording the bar and the image. **BARS: golden 59/59; `dsp_validate` OK on both product graphs; `validate-matrix-contract` MxAdd contiguous 1..6948, 361 families = allowlist, compatibility passed; `bqeverify.sh float` BQE_VERIFY PASS — 0 ULP over 192 cascades x 4 stages x 3 levels x 4 blocks (18,432 words/arm), divergence bitmap 566 of 576 cells matching the model; `mtrverify.sh` METER_BIT_EXACT.** Also corroborated on the part: `ChanGain` and `TalkGain` are applied LINEAR while the masters' Table declares dB (4.0 gives ×4, not ×1.585). **NOT CLAIMED: `busgold` and `conform` were NOT run** — the audio image CHANGED this session, so the earlier byte-identity argument no longer applies and `busgold` is genuinely outstanding; **AuxDelay/GrpGateHold and the rest have no `wire-units.csv` row, so nothing converts them** — the contract's gap, not the mechanism's; the 7 non-passing families were re-run through the SCOPE, not the duplex loop, because the loop is not yet unity. **BENCH LEFT AS FOUND AND BETTER: shipping CPLD bitstream reflashed and IDCODE 0x020a30dd confirmed; the fixed shipping images staged; `config.txt` back on `dtoverlay=dsp4-pcm-slave` with the duplex overlay AND the codec module installed so the bench flag is a one-line flip; backup at `config.txt.pre-duplex-20260908`; matrix-app ACTIVE — it was found stopped.** Write-up `MW/D32/DSP/dsp4-hw-families-20260908.md`; raw `goldens/famverify-20260908b.json`; scorer `golden_harness.py --target hw`; findings S2-1..S2-10; D5 lines rewritten.]   [model: opus]
...
## QUEUED DISPATCH (fire after the desk fillers) — VIRTUAL AUDIO TESTS over the CPLD feedback loop: the golden harness gets a hardware target   [status: 🟡 STEPS 2-4 DELIVERED; THE DUPLEX DEVICE IS UP AND THE LOOP IS ONE PASS-THROUGH AWAY FROM BEING A MEASUREMENT CHANNEL. Every D24 family the landed `defs/products/d24/dsp.csv` addresses is exercised on the part with every parameter resolved BY CELL NAME out of the contract: 14 of 20 pass, 3,028 of 3,698 addressed cells (82 %), 7 of them on the strict bit-exact bar (1,828 cells, 49 %). Harness `tools/pi/dsp4_family_verify.py` + `tools/dsp/landed_map.py`; scorer `golden_harness.py --target hw`; report `MW/D32/DSP/dsp4-hw-families-20260908.md`; per-family lines in D5. Image chip1 a07d3865 / chip2 073d80ef — the baseline plus the wire-unit conversion that fixes ChanGateHold and ChanDelay. STEP 1's loop: the CM4 now enumerates ONE duplex PCM device, S32_LE at 192 kHz, no rate clamp and no xrun, via `invirco,dsp4-pcm-dummy` (`shared/dsp4-logic/pi/dsp4-pcm-dummy/`) — the hub's GPIO17/CS6 ruling worked for voicehat but voicehat is 48 kHz only and this link is 192 kHz. What remains is the main chain: a known word returns on a ~0.28 DC pedestal because 17 sources sum into MIX_MAIN_L and no node was set to bypass. NO LATENCY FIGURE until that is unity. NOT DONE: `busgold` (the audio image CHANGED this session, so the byte-identity argument no longer covers it); the 7 non-passing families re-run through the DUPLEX loop rather than the scope. CPLD carries the SHIPPING bitstream `dsp4_logic.a1f6672af6c3.svf` (md5 dd1e0918, IDCODE 0x020a30dd confirmed); config.txt back on `dtoverlay=dsp4-pcm-slave` with the duplex overlay and codec module installed so the bench flag is a one-line flip; matrix-app ACTIVE]
