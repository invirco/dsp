## HUB DISPATCH 2026-09-08 16:35Z — advance the defs pin to defs-v2026.09.08.2 (your dsp.csv proposal LANDED at the gate), consume `defs/products/<p>/dsp.csv` as the address map, retire `proposals/`, rebuild, golden 59/59 (desk; no bench)   [status: 🔴 dispatched]   [model: sonnet]
...
- DSP address file: **PROPOSED, awaiting the hub gate.**
  `proposals/defs/products/{d24,d32}/dsp.csv` (+ `dsp-unmapped.csv`),
  generated by `gen_dsp.py` from the DSP4 graph and regenerated by
  `regenerate-dsp-contract.sh`. One shared address map (decision D3):
  3,658 cells in both, 0 disagreements. The hub copies them into
  `defs/products/<p>/` and tags; `defs/common/schema/` still declares no
  dsp.csv schema, so the column semantics are proposed in each file's
  header. Nine open questions for PW in
