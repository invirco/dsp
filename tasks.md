...
## HUB DISPATCH 2026-08-24 11:00Z — STRIP FUSION: single-MAC stages, one round/saturate per strip (PW constraints: no FPGA, no block-64; target = 32 basic strips in ONE 21564)   [status: 🟡 FIT MEASURED — 32 basic strips do NOT fit one 21564; they fit the CARD comfortably. Items 1-3 of the dispatch done and measured; items 4-5 overtaken by the answer.
  (1) CLASSES FINISHED. COMP+TUBE converted, bit-exact 0/32 through the whole chain, 7,776 -> 5,747 cycles/block (1.35x). COMP's 'not worth converting' verdict WITHDRAWN: it had only ever been measured as a bare wrap, and _compgain_fx does not clobber all but four registers - it touches r0-r3, r8-r12, i0, leaving r4-r7 and r13-r15.
  (2) FUSION, the part that pays: the biquad cascade now keeps the error feedback IN the MAC accumulator instead of taking the 64-bit result apart and pushing it back every sample. Bit-exact by construction (MRF already holds exactly the value that was being stored), verified 0/32 through all six strip biquads. FILT+EQ 12,060 -> 8,239 cycles/block, 1.46x. STRIP 942 -> 823 cycles/sample; 1,973 at the start of the rewrite, so 2.4x overall.
  (3) SIMD PAIRING WORKS AND IS MEASURED, NOT ASSUMED: 2.39x (2.2-2.6 at +/-1 tick), 0 differing samples of 64, two strips with DIFFERENT coefficients in one instruction stream. PEy verified live on the part first. Saturation had to become a per-PE conditional MOVE - a jump uses PEx's condition for BOTH units, so a branch would saturate strip B whenever strip A clipped.
  THE ANSWER: 32 strips need 249 cycles/sample/strip (fabric at its 40k target). Measured: 823 scalar, 673 with SIMD biquads, 428 with SIMD on all but DLY/RTG, 344 as a perfect-pairing UPPER BOUND = 23.2 strips/chip against 32. No arrangement of the current kernels reaches the goal line. DLY and RTG probably cannot pair at all: SIMD duplicates the compute units and register file but NOT the address generators, and both nodes address per strip.
  WHAT WOULD CHANGE IT: the dynamics are 332 of the 823 cycles/sample and are almost all log2/exp2 polynomial evaluation. Halving them is a NUMERICS change needing D5 sign-off, not a coding change - and even then it is 22 strips/chip, still not 32.
  WHAT IS ALREADY REACHABLE: 32 strips on the CARD. At the realistic 18.6 strips/chip the two fabbed 21564s carry ~37 strips - the full 32-channel product, on the silicon as fabbed, with margin. What it does not give is PW's 'one chip does 32, the second is headroom'.
  FOR PW TO DECIDE: accept 32-on-the-card (no silicon or PCB change, work already measured), or open the numerics question on the dynamics. Items 4 (bus fabric to 40k) and 5 (fused-build ceiling) are still worth doing either way and are unstarted - the fabric number is already assumed at its target in every row above, so it is load-bearing for the card answer too.
  BENCH: production firmware 0df38e8270c14e01ba6ffc57c2122563 / 130ddb0f546966b38ec23b6d9b923748, shipping CPLD untouched, matrix-app active, 3 MCUs verified first try this time. Default image byte-identical throughout (d1c3dd5c / 85d546f9); all fusion behind DSP4_STRIP_FUSED, SIMD behind DSP4_SIMD_PROBE, both default 0]
