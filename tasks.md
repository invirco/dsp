...
R3–R6 — the review hardening items landed as one mechanical pass: gen_dsp_csv.py errors, dsp_validate.py tightening, fixed_ref.py sentinel + knee boundary, and the opportunistic cleanups on each file touched — NO BENCH, NO GRAPH CHANGE, NO IMAGE

WHY. The dsp repo review (tasks.md items R3–R6, review §2.3–§2.7, §3.2–3.5,
patch §5.2–5.5) left four hardening items that need no hardware and no
design decision. They are queued overnight so the bench-bound sessions are
not spent on them. Everything here is repo-only: the rev C unit is NOT
touched, no image is built for it, the SHARC binaries in the tree are not
changed.

RULES. No bench: do not ssh to the rev C unit, do not flash, do not run
sigprofile/famverify. The golden bars that run on the HOST must stay green
before and after every commit: `golden 59/59`, `dsp_validate OK`, and the
float reference checks (`bqeverify` host mode if it has one; otherwise the
python model tests). One commit per item (R3, R4, R5, R6-per-file) so a
revert is surgical. No AI attribution in commits or any work product.

GATES, in order:
1. **R3 — gen_dsp_csv.py.** The GEQ-insertion `next(...)` raises a bare
   StopIteration when the graph is out of sync → a ValueError that names
   the node, the family and the file; `sport_map.json` pre-flight-validated
   ONCE at load (collect every inconsistent entry, report them all) instead
   of asserted per use. Add a test that feeds an out-of-sync graph and
   asserts the message.
2. **R4 — dsp_validate.py.** Duplicate node IDs REJECTED (not reported once
   and re-processed); `parse_id_list()` / `parse_params()` cross-checked
   against the known node-id set and a per-type expected-param-keys table,
   so a malformed dsp.csv row cannot reach codegen. Tests: one malformed
   row per failure class, each refused with a message that quotes the row.
   Run it against the landed `defs/products/d24/dsp.csv` and `d32/dsp.csv`
   — both must still pass; if one does not, that is a FINDING about the
   landed file, reported in the status line, not silently relaxed.
3. **R5 — fixed_ref.py.** `log2_q()` returns a fixed sentinel for `x <= 0`:
   either raise, or prove the sentinel unreachable by legitimate values and
   document the proof next to it — say which and why. Add the soft-knee
   boundary test at `over == ±half_knee`. This file is the SHARC/FPGA
   bit-exactness reference: NO numeric behaviour changes for x > 0 — prove
   it by running the existing reference vectors before and after (byte-
   identical output).
4. **R6 — cleanups, only on files already touched above** (per the item's
   own rule "when touching each file, not as their own dispatch"): gen_dsp
   MCU-only prefixes from config (single registration point with the
   allowlist) + makedirs guards; dedupe parse helpers between
   dsp_validate.py and dsp_simulate.py; hardcode fixed_ref.py's fitted
   polynomial coefficients with the fit script kept; single-pass GEQ
   insertion; marker-based template rewrites with node/file context in the
   error. Anything in R6 whose file was NOT touched in gates 1–3 stays
   queued — list what was left.
5. Regenerate whatever gen_dsp emits into the tree and prove the generated
   output is BYTE-IDENTICAL to before (diff the generated files; the only
   allowed differences are error-path code that never ran). If any
   generated byte changes, STOP and explain.
6. tasks.md: R3–R6 status lines, this block's status; commit + push main.

Bounded: one session; if a gate needs a design call, leave it 🔴 with the
question written and continue to the next gate.

Rules: single trunk — pull main first, commit + push main on completion;
update this block's status (🟢 done / 🔴 blocked) with a short outcome;
no AI attribution in commits or any work product.

## HUB DISPATCH 2026-09-08 22:55Z — CONFIRM THE DSP CODE AGAINST defs-v2026.09.08.3 — pin .3, propose the dsp.csv + dsp-unmapped.csv pair for the 39/51 new GEQ cells + CrossoverSlope 0x0576 (hub lands .4 tonight), both chips rebuilt, whole addressed set verified on the part by name, capacity restated, window-readiness note; no panel-MCU/app deploy   [status: 🟢 done — **CHIP 2'S MARGIN WITH THE REVERB RUNNING IS NOW 5.34 %, MEASURED ON THE 31-BAND IMAGE, AND THAT IS STILL UNDER TEN** — `fxcost.sh`, block 16, two boots, 310,185 = 94.66 % of 327,680 against a restore-and-re-read control that came back within −82 and −19 of a delta of +49,096. The 09-08 figure was 93.16 % / 6.84 %, so **the market bar costs 1.50 points of a margin that was already inside the bar**. That is a product decision, not a defect: six simultaneous reverbs is the worst case the graph can be put in, the shipping default (Type 0 = Echo) leaves **20.26 %**, and the alternative to spending it is not shipping 31-band GEQ. **PROPOSED 54f2924, THE HUB LANDED IT AS defs-v2026.09.08.4 (48745eae5418) AND THE LANDED PAIR IS BYTE-IDENTICAL TO THE PROPOSAL** — d24 dsp.csv 3698→3737 (+39), d32 5358→5409 (+51), dsp-unmapped UNCHANGED at 1248/1590 class for class, `rows + unmapped == cells` at 4985/6999, `check_proposal()` green, `check-contract-drift.sh` clean at `.4`. **BUT THE TWO ASKS IN THIS BRIEF COULD NOT BOTH HOLD, AND THE BRIEF'S OWN NUMBERS SAY WHICH GAVE.** "Every other row byte-identical" and "39/51 new GEQ cells" are mutually exclusive: a GEQ node's SPI block IS its band count and chip 2 packs blocks end to end, so +3 bands on seventeen nodes is +51 words and **1,192 D24 / 1,460 D32 chip-2 addresses MOVE** (2,506 / 3,898 rows byte-identical; chip 1 not one address). **This is the first time this contract has moved an address rather than added one**, so every cached chip-2 address in every host is wrong until it is rebuilt — the panel MCU headers and the app must be built against `.4` **in the same window as the firmware** (`MW/D32/DSP/dsp4-geq31-relayout-20260909.md`). **CrossoverSlope therefore landed at 0x059D, not 0x0576** — the crossover node moved 1397→1436 with everything else — still ONE shared word for all four sections, taking `_xover_coeffs_next[1]`, which no host writes. NOTE FOR THE HUB: the `.4` tag message says 0x0576; the landed row says 0x059D. **BOTH CHIPS REBUILT AND THE IMAGE IS THE SAME BYTES FROM EITHER PROVENANCE**: chip1 **`602a0feb`** / chip2 **`b1325022`**, built once from `proposals/` before the gate and once from `defs/products/` after `.4` landed, identical md5 both times (a new `DSP_LANDED_DIR` makes that honest instead of a hand-edit, and shouts on every run). The 28-band baseline rebuilt from `e278667` reproduces the FX session's `85af9dce`/`bcdbe1f0` exactly, so the two arms are one instrument. **THE MCU DISPATCH-MAP DEFECT DID NOT RECUR**: `mx_dsp_map.h` has **5,409 entries, equal to D32's dsp.csv row count**, and every one of D32's 5,409 and D24's 3,737 addressed cells is in `ghost_cells.c` by name. **THE WHOLE ADDRESSED SET, BY NAME: 17 of 20 families PASSED / 3,619 of 3,737 addressed cells (97 %) / 4 families on the strict bar (1,046 cells)** — `famverify` at pin `defs-v2026.09.08.4`, 0 FAILED, **GEQ 31/31 and CROSSOVER 8/8**. NOT EXERCISED, one line each: AUX_INPUT (8 cells — addresses answer, no stimulus path on this bench), DCA (16 — host-managed by the 08-30 ruling, nothing on the part to probe), METER (94 — no audio probe; `mtrverify.sh` is its bar, not re-run). **That coverage line is the dsp readiness statement for the window.** **BANDS 29-31 ARE REAL ON THE PART**: 12,589 / 15,849 / 19,953 Hz each measured **+12.000 dB against a model of +12.000**, coefficients ≤3 ulp, response ≤0.00014 dB — and `dsp4_geq_verify.py` had hard-coded 28 bands and scored 28 of 31 as a pass on the first run, so it now COUNTS them in the landed map and a hole in the numbering stops it. **THE SLOPE IS SETTABLE AND LR2 WAS WRONG AS SPECIFIED**: the 09-08 proposal's LR2 designs two correct 2nd-order sections whose sum **NULLS at the corner** (−242 dB on the model) because at 2nd order the paths are 180° apart — every 2nd-order Linkwitz-Riley inverts one path, and in the offset encoding that is a sign flip on `b0` alone. **XOVER_DESIGN_OK at BOTH slopes at every corner** (≤4 ulp, each path −6.021 dB, LP+HP flat to 0.00013 dB) with all three negative controls holding: 6, 18 and 3 leave the coefficients unmoved (ignored, not clamped), 12 moves them, and **the frequency stays at 250.0 Hz while the slope is written** — the defect this word was cut to fix. **BARS ON THE FINAL IMAGE**: golden_harness 59/59, dsp_validate OK, busgold **GRAPH BIT-EXACT 0 of 256**, bqeverify float arm **BQE_VERIFY PASS 0 ULP** at block 16, geqverify GEQ_DESIGN_OK ×4, xoververify XOVER_DESIGN_OK, famverify as above, check-contract-drift clean. **`bqeverify`'s FIXED/shootout arms DO NOT LINK** — chip 1 `sec_swco` over by 604 words — and that is **PRE-EXISTING**: the same arm at `e278667` is over by 386, this session's crossover code is 218 of it, and the shipping image links with 85,330 words of chip-1 code free. **CAPACITY, PAIRED IN ONE SESSION ON ONE INSTRUMENT** (block 16, 983.04 MHz, budget 327,680, two boots, minimum, witnessed): chip 2 **256,919 → 261,848**, +4,929 = **+1.50 % of budget**, margin 21.59 % → **20.09 %** — the 09-03 projection for this exact step was +4,940 and **held to eleven cycles**. Chip 1 **262,349 → 261,879**: 470 cycles the wrong way, i.e. it does not move, as the address table said. **Gates 1-6 all done.** Window note: `MW/D32/DSP/dsp4-window-readiness-20260909.md` — artifacts, what the MCU/app build against, the rollback path (`ship_chip1.ldr` `3f0e479a` / `ship_chip2.ldr` `ab43c75b`, untouched), and the one-line acceptance. **NO DEPLOY**: panel MCU not rebuilt or reflashed, app untouched, CPLD still `dsp4_logic.a1f6672af6c3`. Bench start `~/dspboot/chip1.ldr` `5cb4e59b` / `chip2.ldr` `ff36352d`; end **`602a0feb` / `b1325022`** — this session's images are staged there, which is the one thing about the bench that is not as found, and they are the window's artifacts. matrix-app restarted and active.]   [model: opus]
...
WHY. The hub landed `proposals/defs/products/{d24,d32}/{dsp.csv,dsp-
unmapped.csv}` (your commit 2da5619) into `invirco/defs` products/<p>/
unchanged, with schema stubs, and tagged **defs-v2026.09.08.2**
(defs 5696210). The per-product matrix references (`defs/gen/matrix/<p>-
reference.md`, new in .1) now show DSP-addressed cells. This repo is a
consumer: from now on the address map is READ from
`defs/products/<p>/dsp.csv`, never generated here as a source.

DO (desk, mechanical):
1. `git -C defs fetch --tags && git -C defs checkout defs-v2026.09.08.2`;
   `./sync-defs.sh --update-lock` (it must accept the tag); commit the
   gitlink + defs.lock.
2. Make `gen_dsp.py` / `regenerate-dsp-contract.sh` CONSUME
   `defs/products/<p>/dsp.csv` and `dsp-unmapped.csv` as the address map
   (the graph remains the source of the DSP-side node/address facts the
   dsp spoke PROPOSES; the landed file is the contract everything else
   reads). Add a check that a regeneration from the graph equals the
   landed file byte-for-byte (`--check-proposal`) and FAILS loudly if not
   — that is the "proposal drifted from the contract" signal; on failure
   the fix is a new proposal to the hub, never a local edit. Delete
   `proposals/` once the check passes (the landed copy is the record).
3. Rebuild both chips; golden 59/59; dsp_validate OK; images md5 (state
   whether they changed vs 2da5619 and why — they should NOT).
4. tasks.md status + State snapshot (contract defs-v2026.09.08.2), findings;
   commit + push main. No AI attribution in commits or any work product.

Bounded: no bench, no graph changes (C2_SUB_* retirement is a later,
separate dispatch with the ruling), no capacity work.

## HUB DISPATCH 2026-09-08 16:08Z — propose `dsp.csv` for D24 and D32 against defs-v2026.09.08 (ruling 5: dsp proposes, hub lands at the gate) — every generated cell reaches a DSP address or is named; S1-1 main section mapped per the 08-25 ruling as a working assumption   [status: 🟢 done — **EVERY DEFINED CELL IS NOW IN EXACTLY ONE OF TWO FILES, AND THE GENERATOR REFUSES TO RUN IF ONE IS NOT: 5,358 + 1,590 = 6,948 (D32) and 3,698 + 1,248 = 4,946 (D24).** `proposals/defs/products/<p>/{dsp.csv,dsp-unmapped.csv}` are generated from the DSP4 graph by `gen_dsp.py` and regenerated by `regenerate-dsp-contract.sh`; nothing in them is typed. A cell family in neither file stops the generator (`_UNMAPPED_REASONS`), and a graph node holding an SPI address that reaches no cell stops it unless `_UNREACHED_REASONS` says why — S1's "1,011 cells have no DSP mapping (MCU-only or unmapped)", which reads the same as *nobody looked*, can no longer be written. Each written file is READ BACK and compared against the table it came from (5,358 and 3,698 rows identical): it is a definition the hub can land, not a report about one. **S1-1 IS CLOSED AND PW CONFIRMED IT MID-SESSION**: the four chains off `C2_MAIN_XOVER` are MainL / MainR / MainCtr / MainSub on DAC_13..16, `Main[1-1]` is the stereo mix-bus strip, and `C2_SUB_*` is retired — listed with what replaces each node, deleted nowhere. The ruling is corroborated by defs itself and not only relayed: `def_master.py` gates `MainCtr` on `main.ctr` and gates the D24-only **OUT 3 MODE** cell on the same key. **DECISION D3'S ONE SHARED ADDRESS MAP IS NOW STRUCTURAL RATHER THAN PROMISED**: the superset graph is expanded ONCE and each product's file is the intersection with its own cell set — 3,658 shared cells, **0 address disagreements**, 1,700 D32-only, 40 D24-only (all of `MainCtr`, at addresses D32 reserves). D24's own legacy SHARC graph was deliberately NOT used: 201 nodes with no faders, routing, aux, groups, meters or crossover, and deriving addresses from it is exactly the second address map S1 spent a session deleting. **A MAJOR DEFECT FELL OUT: the meter `taps=` declaration was wrong about what the DSP writes.** A meter meters ONE point and lays peak/rms/gr/state at +0..+3, so `taps=` names WORDS; read as tap points it made the four mono main-output meters' RMS word into an `R` channel (two of S1's 23 orphans) and pointed all 32 `Chan*CompMtr001` at base+3 — `_mtr_st[0]`, the meter's own peak-hold state. The RMS word keeps its DISPATCH and loses only its cell (dropping both moved `chip2.ldr`, which is the host losing a real measurement to fix a labelling mistake); CompMtr is listed `unbacked-meter`. **AND THE REWRITE'S OWN FIRST CUT WAS A SILENT BUG**: it parsed the declaration with `parse_params()`, which splits on `;` — the tap separator — so three of the four channel-meter words vanished without a warning; caught by cell counts, fixed with `_parse_taps()`. **BARS: `regenerate-dsp-contract.sh` twice BYTE-IDENTICAL across all 11 artefacts and both proposals; golden 59/59; `dsp_validate` OK ON BOTH product csvs (the three pre-existing D24 errors fixed — `source_gains=0.0` for 32 sources → `source_count=32`, the crosspoints being ROUTING's; `wet`/`dry`/`width` → `mix=30`, dry being 100−wet and `width` not in the declared FX vocabulary); `matrix_gen_id --compare` ALIGNED both at the SAME ids (D24 7322e9a88b18, D32 f09af9c2cd1e) — dsp.csv does not touch MxAdd; `validate-matrix-contract` MxAdd contiguous, 361 families = allowlist; `audit-compat-aliases` UNTOUCHED both products with `alias-audit.md` unchanged; `gen_dsp_csv.py` reproduces the edited `dsp.csv` BYTE-FOR-BYTE, so a regeneration cannot reintroduce `taps=L;R`; and **BOTH IMAGES REBUILD BYTE FOR BYTE against the S1 baseline — chip1 906a70f7 301,580, chip2 3a2d930c 181,908** — only `dsp_params.asm` comments moved.** 73 rows of D32's `_matrix.csv` changed address: 38 `MainSub*` moved onto output chain 4, 35 cleared (32 CompMtr + MainSub Level/Mute/Delay). **NINE OPEN QUESTIONS FOR PW, and Q1 and Q3 are the ones that cost**: Q1 — all four output strips define Level/Mute/Delay and chain N is EQ+COMP+LIM only, so 12 cells reach no word and closing it is four faders + four delay lines on chip 2 at 83.16%; Q3 — every channel defines `CtrOn` (on D32 too, which has no MainCtr) and its word is `_rtg_sub_on_`, an assign to the very bus the ruling retires, so `CtrOn` and the retirement cannot both be right. **NOT CLAIMED: no bench, by dispatch — `busgold` and `conform` not run (the audio image is byte-identical on both chips, so `busgold` cannot have moved, but that is an argument and not a capture). `MW/D24/MX/_matrix.csv` was NOT backfilled with DSP columns — D24's addresses live in the proposal only, and backfilling belongs with the hub landing the file.** Write-up `MW/D32/DSP/dsp4-dspcsv-proposal-20260908.md`; findings S1-1 closed, S1-4..S1-8 new.]   [model: opus]

WHY. PW ruling 2026-09-08 #5: the DSP address file `dsp.csv` (cell → chip /
SPI page / address / ramp profile) is PROPOSED by this repo and LANDED by
the hub at the gate into `defs/products/<p>/dsp.csv`. S1 left it explicit:
8 graph nodes hold SPI addresses no master cell reaches and 23 generated
cells have no matrix row (main bus comp/limiter, L;R meter taps on a mono
strip), plus S1-2's `FxDuckThr` / `PeqGain` reach no DSP address, and
`MW/D24/DSP/SHARC/dsp.csv` fails `dsp_validate` on three pre-existing
parameter errors. Desk work; no bench.

WORKING ASSUMPTION FOR S1-1 (hub, from mx26 commit c97db53, PW ruling
2026-08-25 "main section model"): `Main[1-1]` is the stereo mix-bus strip;
**L / R / Ctr / Sub are each post-crossover OUTPUT strips** (`MainL`,
`MainR`, `MainCtr`, `MainSub`; D24 emits all four since `main.ctr,1`), and
there is NO separate sub mix bus. So graph outputs 1–4 off `C2_MAIN_XOVER`
map to MainL / MainR / MainCtr / MainSub, and the graph's separate
`C2_SUB_*` bus strip is the OLD model — flagged for retirement, NOT deleted
in this session. PW is being asked to confirm; if the hub relays a
different ruling mid-session, follow it.

DO:
1. Author `dsp.csv` for D24 and D32 in the schema `defs/common/schema/`
   declares (read it; if the schema stub is columns-only, propose the
   column semantics in the file header and in the report): one row per
   generated cell that has a DSP-side meaning — chip, SPI page, address,
   ramp profile, scope — derived from the graph (`gen_dsp.py`'s node
   tables), never typed. Every cell in `defs/gen/matrix/<p>-mx-master.csv`
   whose family is DSP-relevant either gets a row or is listed in a
   `not-on-dsp` section with the reason (MCU-only, control-plane, S1-2
   definitions with no DSP behaviour yet). Every graph node address either
   is reached by a cell or is listed as unreached with the S1-1 reason.
2. Fix the three `dsp_validate` parameter errors on the D24 file (state
   each: what was wrong, what the graph says); `dsp_validate` OK on both.
3. Prove: regenerate from the proposed `dsp.csv` → `regenerate-dsp-
   contract.sh` twice byte-identical; golden 59/59; images rebuild (report
   md5s; they MAY change now — that is the point — say which and why);
   `audit-compat-aliases` untouched; `matrix_gen_id --compare` still
   ALIGNED (dsp.csv must not touch MxAdd).
4. Deliver the proposal as files under `proposals/defs/products/<p>/
   dsp.csv` in THIS repo (the hub copies them into defs at the gate and
   tags) plus `MW/D32/DSP/dsp4-dspcsv-proposal-20260908.md` with the
   S1-1 mapping table, the unreached/not-on-dsp lists, and the open
   questions. tasks.md status, findings; commit + push main. No AI
   attribution in commits or any work product.

Bounded: no bench; do not delete `C2_SUB_*` or any graph node; no capacity
work.
...
truth for matrix cells and everything generated; the standalone repo
`invirco/defs` (tag `defs-v2026.09.08`) now holds every product def, the
cell master, the wire declaration and the ONLY matrix expander
(`tools/expand_matrix.py`, `mxc_pack.py`, `matrix_gen_id.py`). mx26 already
consumes it as a submodule. This repo's `sync-from-mx26.sh` + local
`_matrix.csv` regeneration/back-fill produced generation **b4592dfb639e**,
which is NEITHER the tag it pinned (85102bd3097d) NOR current git
(7322e9a88b18) — a second source of truth, and the reason the console, the
DSP firmware and the app are on four different address layouts today
(mx26 pipeline.md "FOUR MATRIX GENERATIONS"). S1 = this repo becomes a
CONSUMER.

DO, in order (desk work; do not touch the rev C unit or the bench):
1. `git submodule add https://github.com/invirco/defs.git defs` pinned to
   tag `defs-v2026.09.08` (use the ssh URL if that is how this machine
   reaches GitHub — check `git remote -v`). Replace `sync-from-mx26.sh`'s
   import with reads from `defs/products/<p>/`, `defs/gen/matrix/<p>-mx-
   master.csv`, `defs/common/cells/mx_master.csv`; `defs.lock` becomes the
   submodule commit + tag + `defs/defs.lock` hashes (no local copies of
   the CSVs). The D24/D32 `_matrix.csv` this repo needs are GENERATED BY
   `defs/tools/expand_matrix.py` — never by anything in this repo; the
   DSP address back-fill (`DspSpi/DspPage/DspAdd/Ramp*`) stays as a
   consumer-side derived table (`gen_dsp.py` output), clearly labelled
   generated, until S2 moves it into a defs emitter. Delete the retired
   sync/import code and any hand-kept copy it replaces.
2. Regenerate everything from the tag: `gen_dsp.py`, `tools/dsp/*`, node
   skeletons, `ghost_cells.h`, `mx_dsp_map.h`. Expect NAME CHANGES since
   the 08-20 pin — Aa*/Zz* sort-prefix purge, main section model
   (MainL/MainR strips), Rtg prefix retirement (RtgLevel→Level etc.), DcaOn
   family — the generators must follow the names; list every hand-written
   reference that broke and how it was fixed (the alias/compat machinery
   this repo carries — `prune-compat-aliases.py`, `audit-compat-aliases.py`
   — is the tool; do not reintroduce aliases for retired names).
3. Prove: `python3 defs/tools/matrix_gen_id.py --compare` of the regenerated
   D24 `_matrix.csv` against `defs/gen/matrix/` expansion = ALIGNED
   `7322e9a88b18`; build both chips; golden 59/59; dsp_validate OK; busgold
   graph bit-exact; W0 controls byte-for-byte; `check-contract-drift.sh`
   passes or is rewritten to check the submodule pin (state which).
   Scoreboard/cost file unchanged unless a name change forces a row rename
   (list them).
4. findings.md ("defs S1"), tasks.md (State snapshot: contract =
   defs-v2026.09.08 via submodule; P3 "contract evolution" item closed as
   superseded), this block's status; commit + push main. No AI attribution
   in commits or any work product.

Bounded: no bench, no capacity work, no dsp.csv authoring (that is the
next dispatch: dsp proposes `dsp.csv`, hub lands it at the gate).

## HUB DISPATCH 2026-09-03 16:16Z — Chip-1 capacity frontier — reduce D22 routing (15-29x over floor), price D25 delay + D20 GAIN→FILT fold, re-measure chip 1 (product already fits; buying headroom)   [status: 🟢 done — **CHIP 1 GOES 88.40% → 79.87% AND STOPS BEING THE TIGHTER PART.** 27,947 cycles/block back, **8.53% of the budget**, margin 11.60% → **20.13%** (65,951 cycles clear), whole graph at block 16 with the full market config active, two boots a point, minimum taken, witnesses clean. Chip 1 was 88.46% against chip 2's 83.16%, tighter by 5.3 points; it is now **79.87% against 83.16%, looser by 3.3**, and the capacity question goes back to chip 2. **(1) D22's 15–29× IS MEASURED, AND IT WAS THE ACCUMULATE RATHER THAN THE PREP THE FINDING BLAMED.** `DSP4_RTG_NOACC=1` deletes the crosspoint accumulate and nothing else: **16,155 c/blk, 4.93% of budget, for 1,024 MACs — 15.78 CYCLES PER MAC against a floor of about one.** The gap is the ACCUMULATOR'S ADDRESS, not the arithmetic — the crosspoint-major form loads a bus's whole 80-bit [lo,hi,ex] triple into MRF, MACs once and writes all three words back, per sample per crosspoint. The fix is a **BUS-MAJOR fabric**: each ROUTING node publishes a COLUMN of `_xpc[bus*32+strip]` at control rate and parks its post-fader block in `_rtg_src` (the pool is reused by the next strip, so a deferred pass cannot read it — that copy is the whole price of deferring); one shared pass, `src/rtg_fabric.asm`, loads each accumulator ONCE per sample and MACs every strip that feeds it. **−10,738 c/blk, −3.28%**, leaving 5.29 c/MAC — a 2.98× improvement — and the cost is now bounded by the number of BUSES rather than by patch density, so it improves further on a real mix rather than this bench's two-crosspoint default. Sends with a non-default pickoff read a tap that is not parked and keep the old sparse path, which is CORRECT rather than merely unused — that is what licenses the dense half to assume one source per strip. **(2) D25's DELAY WAS THE BIGGER LEVER, AND THE 'NEEDS MEASUREMENT' L2 SHARE IS NOW MEASURED: 21,427 c/blk, 6.54%, 41.85 cycles per sample per strip** (`DSP4_DLY_NOMEM=1` deletes the read and keeps everything else) — so the record's 63 c/s minus 8.4 of address arithmetic was indeed memory. The fix deletes nothing: **write the whole block into the delay line, then read the whole block out**, one L2 turnaround instead of BLOCK of them. **−16,857 c/blk, −5.14% — 79% of the entire read cost recovered.** Bit-exact, with the argument written at the loop (sample k reads w−off+k, which for a read inside this block's writes was written at j = k−off < k, already written in the interleaved order too). **(3) D20's REMAINING FOLD PRICES AT ZERO AND IS CLOSED.** `DSP4_GAIN_NOCHAIN=1` replaces GAIN's chain store with a nop — the instruction count the fold would reach, without building the fold — so the difference is the fold's CEILING: **261,718 → 261,743, i.e. −25 cycles, 0.008%, wrong sign. Nothing.** The store sits in the shadow of the float GAIN kernel's serial dependency chain. It would have cost a numeric-spec amendment plus an on-demand post-trim tap for zero, and that is now a measurement rather than an opinion. **(4) THE TWO LEVERS ARE ADDITIVE TO WITHIN NOISE** — 10,738 + 16,857 = 27,595 against 27,947 measured, where the control's own two boots are 809 apart — and **the instrument reproduced itself first**: the control re-read session 26's 289,847 at 289,676, 0.06%. **(5) ONE REAL DEFECT FOUND BY LANDING IT: the crosspoint publish was using `i7`, `l7` and `m7` as scratch, and all three are C-ABI registers.** `c_abi.h`'s C_RUNTIME_INIT sets `m7 = -1` exactly once at boot and every CCALL/C_RETURN reads it without setting it; **`i7` is the C stack pointer, and no node in the tree had ever written it.** Survivable today only because the three C functions are all called from the init path and the whole run loop is assembly on the hardware PC stack — **a trap for whoever adds the first C call or C ISR to the run loop, not a bug anyone would see.** Moved to `i1`/`m4` (ordinary node scratch, 91 sites set it before use); identical instruction count, identical image size, and **the headline arm was RE-MEASURED on the fixed image** (261,729 against 261,718 pre-fix, 0.003%) rather than carried. **BARS: golden 59/59; `dsp_validate` OK both csvs; shipping `src/` regenerates file-for-file identical; `busgold.sh` GRAPH BIT-EXACT with the fabric AND the split on, 0 of 256; a NEW bar — `busgold.sh` gained `DLYOFF` and `dsp4_pairgraph.py` a `--dly`, and the split and interleaved arms captured at DlyOff = 7 (reads landing PARTIALLY inside the block's own writes, the case the DlyOff = 0 golden cannot reach) are 0 of 256 apart, both differing from the golden in all 256 as the positive control; all three W0 witnesses byte for byte plus the float default; and the block-16 profiling image with both flags OFF is BYTE-IDENTICAL to the same build from HEAD on both chips, which is what makes every arm a paired measurement rather than a comparison.** Cost: +164 code words, 1,379 DM words, chip-2 image byte-identical on every arm. `captable.sh` gained `XFLAGS`, which carries an arm's build flags into the build AND into the build directory's name — srckey's reason one level down. No contract file touched, no contract version moved, no D5 change; `check-contract-drift.sh` deliberately not run. NO AUDIO: cycle result, not a listening one. Write-up `MW/D32/DSP/dsp4-chip1-frontier-20260908.md`; costs session 27; scoreboard updated. **STILL OPEN:** (a) the fabric is still ~5× over a one-cycle MAC floor — the residual is the inner loop's load-use interlock and the per-bus triple, and a software-pipelined form is named and not taken; (b) the fabric's advantage on a DENSELY patched console is argued from its cost model, not measured; (c) chip 2 at 83.16% is where the margin work goes next.]   [model: opus]
...
FULL MARKET-CONFIG CAPACITY FIT — cycle measurement only, NO audio (chip 2 is
not audio-configured on this bench; this is the cycle instrument, both chips,
which works as-is). Float is the shipping cascade now (DSP4_BQ_FLOAT default,
chip 2 76.22%); this proves the win holds for the COMPLETE product, not just
32 bare strips.

Goal: build/generate the graph with the FULL shipping feature set and measure
whole-graph cycles on both chips at block 16, to confirm the real product fits
with margin — turning "float fits" into "the whole product fits" (the GEQ
market-bar question, PW's mandate: 31-band GEQ on ALL outputs is table stakes).

1. **Generate the MAX shipping config**: 32 strips with the full per-strip
   chain (GAIN + the biquad EQ + dynamics + everything a strip ships), PLUS
   **31-band GEQ on ALL outputs** (the market bar), plus the master/bus
   processing, metering, and the bus fabric — the complete product graph, not
   a bare-strip minimum. Use the generator's max/shipping product definition
   (author it if the full-config definition doesn't exist yet; state what you
   assumed for output count / bus count and make it a parameter).
2. **Measure whole-graph capacity, both chips, block 16**, the same instrument
   + discipline as the float landing (two boots a point, minimum taken,
   witnesses clean, float kernels default). Report chip 1 % and chip 2 % for
   the full market config, against budget and against the 76.22%/88.56% bare-
   strip float numbers.
3. **Verdict + headroom**: does the complete product fit both chips with
   margin at 983.04 MHz block 16? Report margin-remaining. If it does NOT fit,
   report WHERE the overage is (which functions / how many GEQ instances tip
   it) so PW can decide — do not silently trim the feature set.
4. Note the split: what's on chip 1 vs chip 2 in the full config, and whether
   the 31-band-GEQ-on-all-outputs load lands where there's headroom.

Bars: golden 59/59, dsp_validate OK, busgold graph bit-exact; W0 controls
(DSP4_BQ_FLOAT=0 and the guard flag) still rebuild byte-for-byte. Do NOT touch
the D5/contract. Update the 21564 Kernel Scoreboard. tasks.md + findings;
commit + push main. Do NOT run check-contract-drift.sh as a bar (silently
syncs ~12k lines — known hazard). No audio, no bench-dependent step.

Rules: single trunk — pull main first, commit + push main on completion;
update this block's status (🟢 done / 🔴 blocked) with a short outcome;
no AI attribution in commits or any work product.

## HUB DISPATCH 2026-09-03 07:45Z — LAND FLOAT (ruled) — DSP4_BQ_FLOAT default, offset-form coefficients, on-part bqeverify float arm, GAIN float, update dsp numeric spec   [status: 🟢 done — **FLOAT IS THE SHIPPING CASCADE ON BOTH CHIPS — CHIP 2 76.22%, CHIP 1 88.56%, GUARD-FREE, 0.0080 dB, AND THE KERNEL IS NOW PROVED ON THE PART RATHER THAN MODELLED.** **(1) DEFAULT.** `DSP4_BQ_FLOAT` and the new `DSP4_GAIN_FLOAT` default ON, set in the GENERATOR (`gen_block_header`) as well as build.sh — `src/dsp_block.h` is generated and hand edits to it are overwritten, which cost one build cycle to find. The fixed round-once path and the ‖h‖₁ guard STAY behind `DSP4_BQ_FLOAT=0` as the reference model the FPGA follows, and **all three recorded W0 witnesses still rebuild BYTE FOR BYTE after every change** — `4e89e062`/`4d1d314c`, `23c1e662`/`e45bb82a`, `2249afea`/`3173acb3` — as does the fixed `bqeverify`, both arms. **(2) THE OFFSET WIRE IS BUILT, AND IT IS 0.0080 dB, NOT THE 0.0042 THAT WAS MODELLED.** Five float32 words a stage (b0, n1 = b1+2b0, n2 = b2−b0, c1 = 2+a1, c2 = 1−a2), `_bq_fx_convert_N` still a copy, reconstructed in REGISTERS in the per-stage prologue — five arithmetic instructions and two constant reads, once per stage per BLOCK. **Session 24's 0.0042 reconstructed in longdouble and the part reconstructs at the register file's 32 significand bits**, where `c1 − 2.0` is exact only while c1's lowest set bit is at or above 2⁻³¹; the model now rounds where the part rounds. 0.0080 is still **5.75× under the 0.046 golden bar and 3.3× better than the shipping fixed contract's 0.0265**, against 0.3715 for the direct wire. Running the offset form THROUGH the recursion would reach ~0.003 dB at one more instruction per sample per stage, ~3% of chip 2 — **priced and not taken**. **A SECOND MODEL DEFECT FELL OUT: `bq_float_ref` carried the state's block-boundary crossing and not the SIGNAL's crossing between stages.** The block kernel is stage-outer/sample-inner, so a stage writes its whole block of y to a 32-bit DM buffer and the next stage reads it back — the forward path is 32-bit float even in the 40-bit arm, and so is the entry `FLOAT BY -28`. Worth **nothing** on the response table (0.0080 either way); the 40 bits keep what they were bought for, which is the recursion. **(3) THE ON-PART GAP IS CLOSED: `bqeverify.sh float`, 0 ULP, BLOCK 8 AND BLOCK 16, first run.** New `_bqfd_cascade_simd` (the same float kernel WITHOUT the reconstruction, on the direct wire) is arm B, so the bar is two-sided: both arms hash to `bq_float_ref` over 18,432 and 36,864 words, **the BYPASS cascades agree to the bit and 566/567 of 576 cells diverge exactly as the model names**. A reconstruction that quietly did nothing would show as universal agreement. That one result validates the 32-bit reconstruction, the DM signal buffer, the 40-bit recursion, the block-boundary truncation, the CLIP bound and the FIX rounding mode at once. Harness bug worth recording: the bitmap reader discarded `0xFFFFFFFF` as a dropped SPI transaction, and under float most bitmap words legitimately ARE all-ones — the file's own header warned about exactly this for `_bqev_first`. **(4) GAIN FLOAT: LANDED, AND SMALLER THAN THE ESTIMATE.** `_gsimd_gain_blk`'s audio word is one FLOAT, one multiply, one CLIP and one FIX — 18 instructions per two samples become 11 — while **the METER stays fixed and that is not a compromise**: it wants the pre-clip Q8.24 over-range word and an exact 80-bit sum of squares, and float would make both approximate; the audio's gain is FLOATed from the meter's own Q4.28 word so the two cannot disagree. **Measured whole-graph: 291,606 → 290,193, i.e. 1,413 c/blk, 0.43%, 2.76 c/sample/strip** — the ~6 c/strip estimate was a per-sample figure and the shipping path is SIMD, which had already halved it. **1.28 of that 2.76 is the INTERLEAVE, not the deletion**: FLOAT→multiply→CLIP→FIX is a four-deep serial dependency and returned only 1.49 written as four consecutive instructions; threading the meter's independent max/min/mr1b between them recovered the rest. **The instruction count is not the cycle count.** D20 tap unchanged as ruled — it is the ROUTER's store, not the meter's, and float publishes the same word in the same place. **(5) CAPACITY, block 16, both chips, two boots a point, minimum taken, witnesses clean. Chip 2 264,683 (80.77%) → 249,751 (76.22%), 14,932 cycles freed = 4.56% of budget and 57,188 = 17.45% against the contract. Chip 1 292,863 (89.38%) → 290,193 (88.56%), 2,670 = 0.81%.** The instrument reproduces itself to 0.007% on chip 2 and EXACTLY on chip 1. **The offset reconstruction's cost is measured, not assumed: 3,217 c/blk on chip 2, 0.98%, 5.1 cycles a stage** — the whole price of a 46× accuracy improvement. The float image is also SMALLER than the fixed one (301,580/181,908 against 312,196/191,476). **(6) ONE REAL BUG FOUND BY DOING THIS: `_filt_lpf_A/B` had NO float arm** and was emitted as Q4.28 unconditionally, so under session 24's float build every strip FILT's low-pass stage held `0x10000000` read as a float — 2.5e−29 — and was **SILENT**. It does not show in a cycle measurement because the instruction count is the same either way. Fixed. And the float bypass constant had to change for a sharper reason than session 24's: under the offset encoding five zeros after b0 reconstruct to a **double pole at z = 1**, an integrator squared, not silence — so every bypass initialiser AND every host-wire staging buffer now carries the offset words, including the ones the fixed arm leaves uninitialised. **(7) SPEC UPDATED.** `shared/numeric-spec.md` gets a ruling banner, four **[FLOAT]**-marked sections and a new *The float cascade* section: 40-bit float, offset-form coefficients, the single output clamp, and **the C-wire model — nodes interchange FIXED PCM, not internal format, so SHARC ≡ `bq_float_ref` and FPGA ≡ `fixed_ref` and the two meet at the interchange rather than at bit-identity; no ASRC still holds.** The sample format is still Q4.28 everywhere outside a cascade, which is most of the document. **BARS: golden 59/59; `dsp_validate` OK (666 nodes); `busgold` GRAPH BIT-EXACT on BOTH arms, 0 of 256, sha256 `ba3f52ec` — and the script's own warnings say why that is both stronger and weaker than it reads (bypass biquads, unity gain), which is exactly the coverage `bqeverify float` supplies; `bqeverify float` PASS at block 8 and 16; fixed `bqeverify` PASS both arms; three W0 witnesses byte for byte.** No contract file touched, no contract version moved. Write-up `MW/D32/DSP/dsp4-float-land-20260903.md`; costs in `dsp4-function-costs.csv` session 25; scoreboard artifact updated. **STILL OPEN:** (a) the crossfade path is not bit-identical to steady state under float and cannot be — the per-sample kernel keeps the cascade value in a register where the block kernel passes it through DM; (b) the float arm has never run with a real coefficient swap on a configured chip 2; (c) the offset form IN the recursion, ~0.003 dB for ~3% of chip 2; (d) a PM-resident 40-bit state, ~12 dB on the LF shelf for no cycles in PM, which chip 1 has not got.]   [model: opus]
...
## QUEUED DISPATCH (fire after the desk fillers) — VIRTUAL AUDIO TESTS over the CPLD feedback loop: the golden harness gets a hardware target   [status: 🟡 STEPS 2-4 DELIVERED; STEP 1's LOOP IS A CLEAN PASS-THROUGH THAT DOES NOT PRESERVE SAMPLE ORDER. 14 of 20 families pass on the part, 3,028 of 3,698 addressed cells (82 %), 7 on the strict bit-exact bar; every parameter addressed BY CELL NAME out of the landed dsp.csv. Harness `tools/pi/dsp4_family_verify.py` + `tools/dsp/landed_map.py`; report `MW/D32/DSP/dsp4-hw-families-20260908.md`; D5 lines rewritten. Image chip1 a07d3865 / chip2 073d80ef. LOOP: the duplex device is solved (`invirco,dsp4-pcm-dummy`, one PCM device, S32_LE at 192 kHz, no xrun); the pass-through is real (16 of 17 main-bus sources silenced from the contract, whole chain reads 0x00000000, capture idles at 8 LSB); a known word returns at ratio 1.0000 / 0.9999. **NO LATENCY**: a counter held 64 Pi frames per value returns only 40.4 % of transitions monotonic, dominant step ≈ −9 values — the loop reorders, and a latency through a reordering path is not one. Next probe is `rtl/dsp4_pcm_reframe.v` vs the Pi-input DMA; the ALSA layer is known good. NOT DONE: bit-exactness of the loop (~1.2e-4 residual, unattributed); `conform.sh`. CPLD carries the SHIPPING bitstream `dsp4_logic.a1f6672af6c3.svf` (IDCODE 0x020a30dd confirmed); `dtoverlay=dsp4-pcm-slave` restored and verified after reboot, duplex overlay + codec module left installed for a one-line flip; matrix-app ACTIVE]
...
**MILESTONE 2026-09-03: FLOAT IS THE SHIPPING NUMERIC PATH ON SHARC (PW
ruling).** `DSP4_BQ_FLOAT` and `DSP4_GAIN_FLOAT` default ON. The biquad
cascades are 40-bit float DF-II-T with the coefficient wire carrying D5's
OFFSET encoding as float32 (b0, n1 = b1+2·b0, n2 = b2−b0, c1 = 2+a1,
c2 = 1−a2), reconstructed in registers per stage per block; GAIN's audio
word is float while GAIN's meter stays on the exact wide MAC. **Whole
graph at block 16: chip 2 76.22%, chip 1 88.56%, guard-free**, against
80.77%/89.38% for the fixed arm. **Worst response error 0.0080 dB against
a 0.046 dB bar and the fixed contract's 0.0265** — and it is 0.0080 rather
than the 0.0042 modelled, because the part reconstructs at 32 significand
bits and the model had done it in longdouble. **Numerics are validated ON
THE PART, 0 ULP, block 8 and block 16** (`bqeverify.sh float`), which is
the gap the measurement session named.

**The fixed round-once path and the ‖h‖₁ guard STAY, behind
`DSP4_BQ_FLOAT=0`.** They are normative for the FPGA engine (`fpga/`),
which is fixed-point, and the bar that proves them intact is that all
three recorded W0 witnesses rebuild byte for byte. Do not delete them.
The numeric contract is `shared/numeric-spec.md` (see *The float
cascade*); the landing is `MW/D32/DSP/dsp4-float-land-20260903.md`.

**Open after the landing, in priority order:** (1) a real coefficient
swap on a CONFIGURED chip 2 — the float arm has never run one, and it is
the one path a user gesture reaches; (2) the crossfade path is not
bit-identical to steady state under float and cannot be (the per-sample
kernel keeps the cascade value in a register where the block kernel
passes it through DM) — bound it or document it; (3) the GAIN→FILT
coefficient fold, D20's remaining −17 c/s/strip; (4) re-price the 31-band
GEQ upgrade against the float budget, since float just freed most of the
~8% of chip 2 it needs.
...
- Contract: **defs-v2026.09.08.2 via the `defs` submodule** (`invirco/defs`
  at `5696210`). This repo is a CONSUMER: no definition CSV is copied
  ...
  with the hub; D32 6948 rows, generation `f09af9c2cd1e` — unchanged from
  defs-v2026.09.08 (dsp.csv does not touch MxAdd).
- DSP address file: **LANDED.** `defs/products/{d24,d32}/dsp.csv` (+
  `dsp-unmapped.csv`) is the contract now; `proposals/` is retired
  (deleted, the landed copy is the record). `gen_dsp.py` still expands the
  DSP4 graph into a `cell_map` proposal, but only to run `check_proposal()`
  (`--check-proposal`) — a byte-for-byte, no-fallback proof that the graph
  reproduces every row of the landed files, failing loudly on any drift
  (fix: a new proposal to the hub gate, never a local edit). Once that
  passes, `_matrix.csv` backfill, `ghost_cells.h`, `dsp_params.asm`,
  `mx_dsp_map.h` and `dsp_address_map.md` are all generated from
  `load_landed_address_map()` — the merged `defs/products/<p>/dsp.csv`
  files — not from the graph. Authoring a NEW proposal (only needed once
  the graph itself changes) is now `gen_dsp.py --propose`, off by default.
  One shared address map (decision D3): 3,658 cells in both, 0
  disagreements, 1,700 D32-only, 40 D24-only (MainCtr). `defs/common/
  schema/dsp.csv.md` + `dsp-unmapped.csv.md` now declare the schema (no
  longer proposed per-file). Nine open questions for PW remain open, in
  `MW/D32/DSP/dsp4-dspcsv-proposal-20260908.md`; Q1 (the output strips
  have no fader/mute/delay node) and Q3 (`CtrOn` vs the retired sub bus)
