...
## HUB DISPATCH 2026-09-02 12:38Z — GAIN kernel rewrite — fold polarity/mute to control-rate, block+SIMD, mic-pre tap kept, 22.9->1-2 c/s   [status: 🟢 done — **THE SIMD KERNEL LANDED, BIT-EXACT, AND MEASURED AT BLOCK 16: 552 -> 446 CYCLES/BLOCK/STRIP, 34.5 -> 27.9 c/s, A FACTOR OF 1.24, AND 3,847 CYCLES/BLOCK OFF THE WHOLE OF CHIP 1 = 1.17% OF THE BLOCK-16 BUDGET.** **THE 1-2 c/s TARGET IS NOT REACHED AND IS NOT REACHABLE, AND THE DISPATCH CARRIED A NUMBER ITS OWN RULING HAD ALREADY REJECTED.** The 1-2 comes from `review-dsp-20260828.md`'s GAIN row, which states its condition in the cell: "1-2 (under the GAIN=1MAC fold)" -- the GAIN->FILT coefficient fold that **PW's D20 ruling of the same day REJECTED** because GAIN's output is the clean mic-pre tap. The design and its number were rejected together; the dispatch kept the number. **WHAT GAIN ACTUALLY IS, PER SAMPLE, ONCE POLARITY AND MUTE ARE FOLDED: eighteen instructions, EXACTLY ONE OF WHICH IS THE MULTIPLY** -- 1 gain MAC, 1 rounding-half MAC, 10 of Q4.28 extract-and-saturate (D5), 1 chain store, **1 mic-pre tap store (the D20 feature itself)**, 4 of wide-word meter (PW 2026-08-29). Reaching 1-2 means deleting the numeric contract, one of the two stores and the meter; each is a standing ruling, not an un-done optimisation. GAIN's floor under the rules that presently apply is ~10 instructions/sample, and this session took it most of the way there. **The 2-3%-of-chip-1 expectation was sized off the same rejected row; the measured 1.12-1.17% is stated rather than rounded up.** **TWO OF THE FOUR THINGS THE DISPATCH ASKS FOR WERE ALREADY IN THE TREE**: the ONE effective gain word with polarity as sign and mute as 0.0 has been control-rate since the 2026-08-25 crosspoint mandate, and the hardware-loop block kernel since 2026-08-28. Only the SIMD was missing. **AND WHAT PAIRS IS TWO ADJACENT SAMPLES OF ONE CHANNEL, NOT TWO CHANNELS -- DELIBERATELY.** Channel pairing needs its operands interleaved and `_bq_pair_blk` pays ~4 cycles/sample/strip to gather and scatter, which a 51-cycle cascade can afford and a 20-instruction kernel cannot. **A gain is MEMORYLESS**, so the pair PEx and PEy need is already contiguous in the block the previous node wrote: `dm(i0, 2)` feeds both units with no gather, no scratch, no scatter. Same factor, none of the price. **BIT-EXACT BY CONSTRUCTION AND PROVED FOUR WAYS.** The audio path is per-sample independent; the meter's sum of squares splits across the two units and is recombined EXACTLY (MRF is an 80-bit integer accumulator, no rounding, no saturation, 16 samples top out near 2^66, and integer addition is associative) in `_gsimd_flush`, which tail-calls `_mtr_flush` so the node pays for ONE call, the same one it already paid. Bars: **`busgold` GRAPH BIT-EXACT 0 of 256** on the golden's own sha256; **`mtrverify` METER_BIT_EXACT** with ms64/pk_lo/pk_hi all exact against `fixed_ref` -- and its wide-word control drives GAIN at a NON-ROUND 0.4970 (Q4.28 0x07F3B648), matching the wide model and rejecting the narrow; **golden 59/59, and 59/59 again at `DSP4_GEN_BLOCK=16`**; **`numverify` 57 of 57** with its negative control two-sided. **THE KERNEL WAS WRONG ON ITS FIRST CUT AND busgold RETURNED 0 OF 256 ANYWAY -- THAT IS THE MOST USEFUL THING IN THE SESSION.** `_GAIN_BLK_COMMON` loads the rounding half and the saturation mask with PEYEN still DOWN, which writes PEx's copy and leaves PEy's at zero, so PEy would have TRUNCATED its samples and saturated against a mask of 0. **busgold cannot see either**: its stimulus is a +/-0.5 square at UNITY gain, whose Q4.28 word is 2^28 exactly, so every product's low 28 bits are already zero and truncation equals rounding on every sample -- and |x|=0.5 against a gain under 8 can never saturate. Found by reading register lifetimes, not by a measurement, and recorded that way. **SO THE BAR WAS REBUILT TO SEE IT.** New `SHARC/gainsimd.sh` sets a NON-ROUND gain (0.70710678, Q4.28 0x0B504F33) and compares arm against arm: **simd vs scalar 0 of 256, BIT-EXACT**, capture digest `8f46bd9a` -- different from the unity capture, which is the proof the gain reached the audio. `dsp4_pairgraph.py` gained `--gain` for it and its `--compare` now **WARNS when both captures were taken at unity**, so a blind verdict says so on its own. **THE NEGATIVE CONTROL FIRES WITH A DIAGNOSTIC SIGNATURE**: `DSP4_GAIN_SIMD_NEGCTL=1` zeroes PEy's gain word and moves 239-245 of 256 words with **exactly 128 of 256 non-zero**, first difference at word 1 -- half the samples silenced, the odd-sample signature, which is direct evidence PEYEN is up and the second unit is computing them. A kernel that never set PEYEN would have agreed with the scalar arm perfectly, which is the silent-fallback trap c2dyngold was caught by. **THE CONTROL IS PROVED, NOT ASSUMED**: `DSP4_GAIN_SIMD=0` on the new tree rebuilds the PREVIOUS tree BYTE FOR BYTE (chip1.ldr e95cdfe8 / chip2.ldr 56037cf9), so the measured difference is the kernel and nothing else. **W0 UNMOVED: 23c1e662 / e45bb82a, 301,764 / 182,092 bytes**, reproduced before and after -- the shipping default is `DSP4_BLOCK_KERNELS=0` and every new var, extern and body is behind that guard. **THE INSTRUMENT IS CALIBRATED THREE WAYS BEFORE ANY VERDICT IS READ OFF IT.** New `SHARC/gainprof.sh` (sigprofile's ladder from a `DSP4_GEN_BLOCK=16` scratch tree, sweeping the flag, two boots a point, minimum taken, pool parity witnessed `even` throughout): its scalar whole-graph point reads 308,174 against session 19's 307,866 for chip 1 at block 16, **0.1% apart from another session and another script**; points reproduce across boots to 7 cycles on a 17,000 baseline; and **the two arms read the SAME NUMBER at limit 1 -- 16,521 and 16,521 -- where no GAIN node runs**, so a systematic offset between arms would have shown there. The whole-graph reading (32 gain nodes of signal) gives 3,847 min-to-min, 3,663 boot-2 to boot-2, against within-arm spreads of 863/1,047; **the class ladder scaled by 32 predicts 3,392, so the two instruments are ~10% apart and BOTH are reported.** **THE PRICE NOBODY HAD PRICED IS PROGRAM MEMORY, AND IT IS THE BINDING ONE. The kernel costs 1,486 BYTES of chip 1's PM: free space goes 3,328 -> 1,842 bytes on the block-16 32-strip build.** **It has already broken something: the `DSP4_PROFILE_SIGNAL=1` whole-graph image no longer links**, overflowing sec_swco by 736 words in the SIMD arm and linking in the scalar arm -- which is why the whole-graph figure is taken on the SILENCE graph (GAIN's cost is signal-independent, so the arm-to-arm difference carries; the absolute totals are silence totals and are NOT comparable to the capacity table's). 512 bytes were recovered in-session by loading the constants once, inside the region, instead of once outside and once in. **A FURTHER ~900 BYTES IS AVAILABLE AND WAS NOT TAKEN**: sharing the PEYEN prologue the way `_gsimd_flush` is shared costs one more call/rts per node per block = 0.94 c/s = **13% of the whole cycle win**. Chip 1 is at 94.0% of budget AND at 1,842 bytes of code space; which is the scarcer resource is a product question, so it is put to PW rather than decided here. **TWO BARS IN THE TREE WERE UNRUNNABLE AND ARE NOW FIXED**: `busgold.sh` and `conform.sh` both referenced `$ROOT` on the line building `BLOCKPY` and assigned `ROOT=` two lines LATER, so under `set -u` both aborted with `ROOT: unbound variable`. As committed before this session neither could run. **AND `check-contract-drift.sh` FAILS ON A CLEAN TREE ON DRIFT IT CREATES ITSELF** -- reported, not fixed. It rewrites both `_matrix.csv` and both `*-mx-master.csv` and **APPENDS `main.ctr,1` to `MW/D24/DEFS/d24.csv`**, a synced contract file CLAUDE.md's first hard rule says is never hand-edited, then fails on the hash of the line it just added (`D24_DEF_SHA256 expected 8c5c9ea1 actual 39758827`). Confirmed by reverting to clean, running the checker alone, and finding the same five files dirty again; `regenerate-dsp-contract.sh` does the same. Left alone deliberately -- taking the new hash would launder whatever real drift is underneath -- and its churn was REVERTED rather than committed. **This change touches no contract file; `dsp_validate.py` OK; no contract version moves.** **STILL OPEN, SAID PLAINLY**: (a) one instruction of multifunction packing is available in the loop and was NOT taken -- `mrf = mrf + r12*r12 (ssi), r0 = dm(i0, 2)` assembles and folds the load into the meter's MAC, ~5% of the loop, left out rather than make every measurement here stale for 5%; (b) the control-rate gate (`ctl_gate()`, which RTG uses) was not extended to GAIN -- worth ~1.5 c/s, and getting its busy condition wrong FREEZES A FADER MOVE, which is a `conform.sh` matter, not an end-of-session one; (c) `gainsimd.sh` and `mtrverify` run at block 8 (the repo tree's size) -- no bit-exactness bar was run at block 16 this session; (d) the whole-graph number is a silence number; (e) the GAIN class figures INCLUDE the strip's meter node, as the block-8 row already did, because a meter takes its source's ladder position -- it is identical in both arms and cancels out of the 106. Write-up: `MW/D32/DSP/dsp4-gain-simd-20260902.md`; costs in `dsp4-function-costs.csv` session 20.]   [model: opus]
