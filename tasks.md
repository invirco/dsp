## HUB DISPATCH 2026-09-08 16:48Z — VIRTUAL AUDIO over the CPLD loop, continued: the golden harness gets its hardware target (steps 2–4 of the queued block), on the float image at defs-v2026.09.08.2 — every kernel family exercised through the real SHARC path, per-family hardware-verified lines, and the coverage fraction reported   [status: 🟡 partial — **EVERY D24 FAMILY THE LANDED CONTRACT ADDRESSES IS NOW EXERCISED ON THE PART, AND 13 OF 20 PASS: 2,934 OF 3,698 ADDRESSED CELLS, 79 %.** Four of those pass on the strict bar — the part's own captured samples reproduced WORD FOR WORD by the reference model with its deliberately-wrong twin required to disagree — covering 1,046 cells (28 %): GATE, COMPRESSOR, TUBE_SAT, FADER_PAN. **THIS IS THE CONTRACT'S FIRST LIVE USE AND IT HELD.** `tools/dsp/landed_map.py` reads `defs/products/d24/dsp.csv` and stages it beside the images carrying the `defs.lock` pin and the CSV sha256 (the Pi has no `defs` checkout); `tools/pi/dsp4_family_verify.py` resolves EVERY address it writes by CELL NAME out of that file — no stride arithmetic anywhere. **All 25 constants the bench harness had transcribed out of `dsp_address_map.md` agree with the landed address for their cell, and every landed address written ANSWERED** (`SPI_ERR_COUNT` read either side of every write; no `NO_ENTRY` on any family, both chips). Image: the SHIPPING FLOAT build, plain `./build.sh`, **chip1 `906a70f7` 301,580 / chip2 `3a2d930c` 181,908 — byte for byte the session's starting baseline**, so the build is its own W0 check. **A MAJOR DEFECT: `ChanGateHold` reaches the kernel UNCONVERTED and a gate that has opened never closes again.** `_gate_hold_*` is an integer sample count (initialiser 2400 = 50 ms) and the SPI dispatch stores the host's float32 word into it raw, so the documented 1.0 ms lands 0x3F800000 = 1,065,353,216 samples ≈ 6.2 hours. Measured: with that hold and the threshold at 0 dBFS over a −6 dBFS step the gate stayed OPEN (`_gate_gain_target_q` unity, `_buf_C1_GATE_01` 0x07FFFF07); written as a raw 48 it shuts (target 2684355 = the range floor, peak 0x00147BDB). The ladder itself is bit-exact — the missing conversion is the whole fault, and `wire-units.csv` already carries the row that predicted it. **FOUR FAMILIES ANSWER EVERY LANDED ADDRESS AND REACH NO SAMPLE — 646 cells: ANTI_FB, GEQ, CROSSOVER, FX_ENGINE**, proved by walking the chain with an impulse and diffing consecutive node buffers (0 of 32 words differ at every node; the crossover does not split at any frequency; the FX engine passes an impulse through with On=1 and Mix=100). ANTI_FB and FX_ENGINE are CORROBORATED by the D38 static list — this is the live confirmation it was waiting for — but **GEQ and CROSSOVER are NOT on that list**, so the D38 count of 896 is low by at least 372 cells. METER FAILS: `_mtr_peak_C1_MTR_01` reads 0x40E1AFA1 against a captured post-trim peak of exactly 0.5 — 7.0527 as float32, 4.0551 as Q4.28, neither of them the peak. **THREE INSTRUMENT FAULTS FOUND, EACH OF WHICH HAD ALREADY PRODUCED A WRONG ANSWER ABOUT THE FIRMWARE, all fixed**: `BQCVT` scored the FLOAT image against the FIXED reference and reported EQ/FILT FAILED (part held float 1.0, model predicted Q4.28 1.0) — the float arm's bar is `bqeverify.sh float` vs `bq_float_ref`; `dsp4_node_verify.vpeek`'s zero sentinel is armed in `main()`, so any tool calling `run_node()` directly read every genuinely-ZERO parameter as unreadable and COMPRESSOR lost its verdict twice on its hard-knee words; and a DC step cannot see a peaking section at all, which read GEQ/ANTI_FB/CROSSOVER as inert before the frequency-shaped families were moved onto an impulse. Also corroborated on the part: `ChanGain` and `TalkGain` are applied LINEAR while the masters' Table declares dB (4.0 gives ×4, not ×1.585). **THE CM4 DUPLEX BLOCKER: the recorded fix direction is CONFIRMED CORRECT and still blocked on a codec, not on the DSP.** One dai-link DOES give one duplex PCM device — `asahi-kasei,ak4554` enumerates `00-00 … playback 1 : capture 1` — but ak4554's DAI is **S16_LE only** and the lanes are 32-bit; `google,voicehat` has exactly the right DAI (2 ch, 48 kHz, S32_LE, both ways) and refuses to probe because `sdmode-gpios` is mandatory; `dit`+`dir` as two codecs on one link comes out CAPTURE ONLY. Way out: name a free CM4 GPIO for voicehat's sdmode (a carrier question — with a pin this is a three-line overlay) or carry our own bidirectional dummy. Overlay and all three results in `shared/dsp4-logic/pi/dsp4-pcm-duplex.dts`. **BARS: golden 59/59; `dsp_validate` OK on both product graphs (D32 666 nodes, D24 201 nodes); both images rebuild byte for byte against the baseline.** **NOT CLAIMED: `busgold` and `conform` were NOT run** (the audio image is byte-identical on both chips so `busgold` cannot have moved, but that is an argument and not a capture); **`bqeverify.sh float` was NOT run**, so EQ/FILT/GEQ/CROSSOVER/ANTI_FB have no bit-exact float verdict this session and pass on reachability only; **`mtrverify.sh` was NOT run**, so METER's failure is a readback disagreement and not yet a diagnosis; the Pi audio loop is still not a measurement channel and no latency figure is quoted. **BENCH LEFT AS FOUND AND BETTER: shipping CPLD bitstream `dsp4_logic.a1f6672af6c3.svf` reflashed and IDCODE 0x020a30dd confirmed; shipping float images staged (906a70f7/3a2d930c); the original `dsp4-pcm-slave` overlay restored in config.txt (backup `config.txt.pre-duplex-20260908`); matrix-app ACTIVE — it was found stopped.** Write-up `MW/D32/DSP/dsp4-hw-families-20260908.md`; raw `MW/D32/DSP/SHARC/goldens/famverify-20260908.json`; scorer `tools/dsp/golden_harness.py --target hw`; findings S2-1..S2-7; D5 lines added.]   [model: opus]
...
## QUEUED DISPATCH (fire after the desk fillers) — VIRTUAL AUDIO TESTS over the CPLD feedback loop: the golden harness gets a hardware target   [status: 🟡 STEPS 2-4 DELIVERED THROUGH THE DSP-SIDE SCOPE, STEP 1's PI LOOP STILL BLOCKED. Every D24 family the landed `defs/products/d24/dsp.csv` addresses is exercised on the part on the shipping FLOAT image (chip1 906a70f7 / chip2 3a2d930c) with every parameter resolved BY CELL NAME out of the contract: 13 of 20 families pass, 2,934 of 3,698 addressed cells (79 %), 4 of them bit-exact against the reference model with negative controls firing. Harness `tools/pi/dsp4_family_verify.py` + `tools/dsp/landed_map.py`; scorer `golden_harness.py --target hw`; report `MW/D32/DSP/dsp4-hw-families-20260908.md`; per-family lines in D5. The measurement channel is `src/scope.asm`, NOT the Pi audio path, and that is unchanged: the CM4 still cannot stream duplex. The recorded fix direction (one dai-link, a codec declaring both directions) is CONFIRMED — ak4554 gives a real `playback 1 : capture 1` device — but ak4554 is S16_LE only, voicehat needs a mandatory sdmode GPIO, and dit+dir on one link comes out capture-only. Needs a free CM4 GPIO named, or our own bidirectional dummy codec. NOT DONE: `bqeverify.sh float` (the float cascade's bit-exact bar), `mtrverify.sh` (METER), `busgold`, and no latency figure. CPLD carries the SHIPPING bitstream `dsp4_logic.a1f6672af6c3.svf` (md5 dd1e0918, IDCODE 0x020a30dd confirmed) — the loopback bitstream was flashed for the duplex attempt and restored; matrix-app is ACTIVE]
