...
| **golden nodes** | `cd MW/D32/DSP/SHARC && ./goldnode.sh` | `NODE VERIFY BIT-EXACT` — every captured sample of GATE, COMP, TUBE and FADER_PAN matches `fixed_ref` from the node's own converted parameters, **and every negative control fires**. This is the only bar that measures a strip NODE rather than a primitive or a library routine, and it drives the SHIPPING GRAPH: what it captures is the emitted node body, not a probe copy of it. Its converted-parameter rows are a bar in their own right — they are where D39's dB range and D40's percent blend went wrong, and a mismatch there stops the sample-path measurement rather than letting it run on the wrong coefficients. Plain `./build.sh`, so the md5 it prints is also the session's W0 check. |
| model vs float64 | `python3 tools/dsp/golden_harness.py` | **58/58**. Costs no bench time and no boot; run it on every session that touches `fixed_ref.py`, `boundary_vectors.py` or a generator that emits arithmetic. |
