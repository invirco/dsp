...
| `MW/D32/DSP/SHARC/dcapar.sh` | The D57/D59 cell-semantics evidence on the part — RtgDca assigns rather than scales, and CompPar's default leaves the compressor wet. Runs against either image (`BUILD=0` uses whatever is on the bench), so both fixes have a before |
